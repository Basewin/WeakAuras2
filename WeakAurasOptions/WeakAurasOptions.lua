...
local GetSpellInfo, GetItemInfo, GetItemIcon, UnitName =
  GetSpellInfo,
  GetItemInfo,
  GetItemIcon,
  UnitName
    ...
    if index then
      ...
      for index, id in pairs(parentData.controlledChildren) do
        ...
        childButton:SetGroup(
          parentData.id,
          parentData.regionType == "dynamicgroup"
        )
...
AceGUI:RegisterLayout("ButtonsScrollLayout", function(
content,
  children,
  skipLayoutFinished
)
...
function WeakAuras.ConstructOptions(
prototype,
  data,
  startorder,
  triggernum,
  ...
  elseif data.triggers[triggernum] then
    if (triggertype == "untrigger") then
      ...
      trigger, untrigger =
        data.triggers[triggernum].trigger,
        data.triggers[triggernum].untrigger
    end
  else
    error(
      "Improper argument to WeakAuras.ConstructOptions - trigger number not in range"
    )
  end
  ...
  for index, arg in pairs(prototype.args) do
    local hidden = nil
    if (arg.collapse and isCollapsedFunctions[arg.collapse] and type(
      arg.enable
    ) == "function") then
    ...
    if (name and arg.type == "collapse") then
      options["summary_" .. arg.name] = {
        ...
        name = type(arg.display) == "function" and arg.display(
          trigger
        ) or arg.display,
        order = order,
        fontSize = "medium"
    ...
    elseif (name and not arg.hidden) then
        ...
        options["use_" .. name] = {
          ...
          name = function(input)
            ...
            elseif (value == false) then
              return "|cFFFF0000 " .. L["Negator"] .. " " .. arg.display
            else
              return "|cFF00FF00" .. arg.display
            end
          end,
          desc = arg.desc,
          get = function()
      ...
      elseif (arg.type == "spell" or arg.type == "aura" or arg.type == "item") then
        if (not arg.required or triggertype ~= "untrigger") then
          ...
          options["icon" .. name] = {
            ...
            disabled = function()
              return not ((arg.type == "aura" and trigger[realname] and spellCache.GetIcon(
                trigger[realname]
              )) or (arg.type == "spell" and trigger[realname] and GetSpellInfo(
                trigger[realname]
              )) or (arg.type == "item" and trigger[realname] and GetItemIcon(
                trigger[realname]
              )))
            end
          }
          ...
          options[name] = {
            ...
            get = function()
              ...
              elseif (arg.type == "spell") then
                local useExactSpellId =
                  (arg.showExactOption and trigger["use_exact_" .. realname]) or arg.forceExactOption
      ...
      elseif (arg.type == "select" or arg.type == "unit") then
        ...
        if (arg.type == "unit" and not (arg.required and triggertype == "untrigger")) then
          options["use_specific_" .. name] = {
            ...
            order = order,
            hidden = function()
              return (not trigger["use_specific_" .. realname] and trigger[realname] ~= "member") or (type(
                hidden
              ) == "function" and hidden(trigger)) or (type(
                hidden
              ) ~= "function" and hidden)
            end,
            get = function()
      ...
      elseif (arg.type == "multiselect") then
        ...
        options[name] = {
          ...
          values = values,
          hidden = function()
            return (type(hidden) == "function" and hidden(trigger)) or (type(
              hidden
            ) ~= "function" and hidden) or trigger["use_" .. realname] == false
          end,
          disabled = function()
            return not trigger["use_" .. realname]
          end,
  ...
  if not (triggertype or prototype.automaticrequired) then
    ...
    if (unevent == "custom") then
      local unevent_options =
        WeakAuras.ConstructOptions(
          prototype,
          data,
          order,
          triggernum,
...
function WeakAuras.MultipleDisplayTooltipMenu()
  ...
  local menu = { {
    ...
    func = function()
      local data = {
        id = WeakAuras.FindUnusedId(
          tempGroup.controlledChildren[1] .. " Group"
        ),
        regionType = "group"
      }
      WeakAuras.Add(data)
...
function WeakAuras.ConfirmDelete(toDelete, parents)
  if toDelete then
    ...
    StaticPopupDialogs["WEAKAURAS_CONFIRM_DELETE"].text =
      warningForm:format(#toDelete)
    StaticPopup_Show("WEAKAURAS_CONFIRM_DELETE", "", "", {
      toDelete = toDelete,
      parents = parents
    })
  end
...
local function disabeldOrHiddenChild(childOptionTable, info)
  return hiddenChild(childOptionTable, info) or disabledChild(
    childOptionTable,
    info
  )
end
            ...
            if first then
              ...
              if (#combinedValues == #values) then
                for j = 1, #combinedValues do
                  if (type(combinedValues[j]) == "number" and type(
                    values[j]
                  ) == "number") then
                    if ((math.floor(
                      combinedValues[j] * 100
                    ) / 100) ~= (math.floor(values[j] * 100) / 100)) then
                      same = false
                      break
                    end
                  else
...
local function setAll(data, info, ...)
  ...
  for index, childId in ipairs(data.controlledChildren) do
    ...
    if childData then
      ...
      if (childOption and not disabeldOrHiddenChild(
        childOptionTable,
        info
      )) then
        for i = #childOptionTable, 0, -1 do
    ...
    if (#tableA == #tableB) then
      for j = 1, #tableA do
        if (type(tableA[j]) == "number" and type(tableB[j]) == "number") then
          if ((math.floor(tableA[j] * 100) / 100) ~= (math.floor(
            tableB[j] * 100
          ) / 100)) then
            return false
          end
          ...
          if sameAll(info) then
            ...
            for index, childId in ipairs(data.controlledChildren) do
              ...
              if childData then
                ...
                if (childOption and not hiddenChild(
                  childOptionTable,
                  info
                )) then
                  for i = #childOptionTable, 0, -1 do
                    if childOptionTable[i].get then
                      if (intable.type == "toggle") then
                        ...
                        if (tri and childOptionTable[i].get(info)) then
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r" .. name
                          )
                        elseif tri then
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r" .. L["Ignored"]
                          )
                        elseif childOptionTable[i].get(info) then
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r|cFF00FF00" .. L["Enabled"]
                          )
                        else
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r|cFFFF0000" .. L["Disabled"]
                          )
                        end
                      ...
                      elseif (intable.type == "select") then
                        local selectValues =
                          type(
                            intable.values
                          ) == "table" and intable.values or intable.values(
                            info
                          )
                        ...
                        if intable.dialogControl == "LSM30_Font" then
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r" .. key
                          )
                        else
                          tinsert(
                            values,
                            "|cFFE0E000" .. childId .. ": |r" .. display
                          )
                        end
                      elseif (intable.type == "multiselect") then
                        ...
                        tinsert(
                          values,
                          "|cFFE0E000" .. childId .. ": |r" .. table.concat(
                            selectedValues,
                            ","
                          )
                        )
                        ...
                        local display =
                          childOptionTable[i].get(info) or L["None"]
...
function WeakAuras.AddCodeOption(
args,
  data,
  name,
  prefix,
  ...
  args[prefix .. "_custom"] = {
    ...
    arg = { extraFunctions = extraFunctions },
    set = function(info, v)
      local subdata = data
      for i = 1, #path - 1 do
        local key = path[i]
...
local function addCollapsibleHeader(options, key, input, order, isGroupTab)
  if input.__noHeader then return end
  ...
  local titleWidth =
    WeakAuras.doubleWidth - (hasDelete and 0.15 or 0) - (hasUp and 0.15 or 0) - (hasDown and 0.15 or 0) - (hasDuplicate and 0.15 or 0)
  ...
  options[key .. "collapseButton"] = {
    ...
    func = function(info)
      if not nooptions then
        local isCollapsed =
          WeakAuras.IsCollapsed("collapse", "region", key, false)
  ...
  if hiddenFunc then
    return function()
      return hiddenFunc() or WeakAuras.IsCollapsed(
        "collapse",
        "region",
        key,
        false
...
local function flattenRegionOptions(allOptions, isGroupTab)
  ...
  for optionGroup, options in pairs(allOptions) do
    ...
    local collapsedFunc =
      addCollapsibleHeader(result, optionGroup, options, groupBase, isGroupTab)

    for optionName, option in pairs(options) do
      if not optionName:find("^__") then
        result[optionGroup .. "." .. optionName] =
          copyOptionTable(option, groupBase, collapsedFunc)
      end
    end
  end
...
local function AddSubRegionImpl(data, subRegionName)
  ...
  if WeakAuras.subRegionTypes[subRegionName] and WeakAuras.subRegionTypes[subRegionName] then
    if WeakAuras.subRegionTypes[subRegionName].supports(data.regionType) then
      ...
      local subRegionData =
        type(default) == "function" and default(data.regionType) or CopyTable(
          default
        )
...
local function AddOptionsForSupportedSubRegion(regionOption, data, supported)
  if not next(supported) then return end
  ...
  for subRegionType in pairs(supported) do
    if WeakAuras.subRegionTypes[subRegionType].supportsAdd then
      ...
      result[subRegionType] = {
        ...
        name = string.format(
          L["Add %s"],
          WeakAuras.subRegionTypes[subRegionType].displayName
        ),
        order = order,
        func = function()
...
function WeakAuras.AddOption(id, data)
  ...
  if regionOptions[data.regionType] then
    ...
    if data.subRegions then
      ...
      for index, subRegionData in ipairs(data.subRegions) do
        ...
        if WeakAuras.subRegionOptions[subRegionType] then
          hasSubElements = true
          subIndex[subRegionType] =
            subIndex[subRegionType] and subIndex[subRegionType] + 1 or 1
          local options, common =
            WeakAuras.subRegionOptions[subRegionType].create(
              data,
              subRegionData,
              index,
              subIndex[subRegionType]
    ...
    hasSubElements =
      AddOptionsForSupportedSubRegion(
        regionOption,
        data,
        supported
      ) or hasSubElements
    ...
    regionOption = {
      [data.regionType] = {
        ...
        unsupported = {
          type = "description",
          name = L['This region of type "%s" is not supported.']:format(
            data.regionType
          ),
          order = 2
        }
...
function WeakAuras.DeleteConditionsForTrigger(data, triggernum)
  for _, condition in ipairs(data.conditions) do
    ...
    if (condition.check and condition.check.checks) then
      DeleteConditionsForTriggerHandleSubChecks(
...
function WeakAuras.ReloadTriggerOptions(data)
  ...
  if data.controlledChildren then
    ...
    local commonOptionTriggerChoice =
      optionTriggerChoices[id] >= 1 and optionTriggerChoices[id]
    for index, childId in pairs(data.controlledChildren) do
      local childData = WeakAuras.GetData(childId)
      if childData then
        optionTriggerChoices[childId] =
          commonOptionTriggerChoice or optionTriggerChoices[childId] or 1
        WeakAuras.ReloadTriggerOptions(childData)
      end
    end
    -- TODO: remove this once legacy aura trigger is removed
  else
    optionTriggerChoices[id] =
      min(optionTriggerChoices[id] or 1, #data.triggers)
      ...
      for index, childId in pairs(data.controlledChildren) do
        ...
        if childData then
          if #childData.triggers > 1 then
            ...
            WeakAuras.DeleteConditionsForTrigger(
              childData,
              optionTriggerChoices[childId]
            )
            optionTriggerChoices[childId] =
              max(1, optionTriggerChoices[childId] - 1)
            WeakAuras.ReloadTriggerOptions(childData)
          end
        end
      end
  ...
  local function moveTriggerDownImpl(data, i)
    ...
    data.triggers[i], data.triggers[i + 1] =
      data.triggers[i + 1],
      data.triggers[i]
    for _, condition in ipairs(data.conditions) do
  ...
  local trigger_options = {
    ...
    activeTriggerMode = {
      ...
      values = function()
        local vals = {}
        vals[WeakAuras.trigger_modes.first_active] =
    ...
    chooseTrigger = {
      ...
      get = function()
        return optionTriggerChoices[id]
      end,
      set = function(info, v)
  ...
  WeakAuras.AddCodeOption(
    ...
    "https://github.com/WeakAuras/WeakAuras2/wiki/Aura-Activation-and-Deactivation",
    0.1,
    hideTriggerCombiner,
  ...
  if data.controlledChildren then
    ...
    for index, childId in ipairs(data.controlledChildren) do
      ...
      local childData = WeakAuras.GetData(childId)
      local trigger =
        triggerChoice and childData.triggers[triggerChoice].trigger
      local triggerSystemOptionsFunction =
        trigger.type and WeakAuras.triggerTypesOptions[trigger.type]
    ...
    if commontriggerSystemOptionsFunction then
      trigger_options =
        union(
          trigger_options,
          commontriggerSystemOptionsFunction(data, optionTriggerChoices)
        )
    end
    if anyOldAuraTriggers then
      trigger_options =
        union(
          trigger_options,
          WeakAuras.GetBuffConversionOptions(data, optionTriggerChoices)
        )
    end
    ...
    displayOptions[id].get = function(info, ...)
    ...
    displayOptions[id].hidden = function(info, ...)
      return hiddenAll(data, info, ...)
    end
    displayOptions[id].disabled = function(info, ...)
    ...
    displayOptions[id].args.load.args =
      WeakAuras.ConstructOptions(
    ...
    WeakAuras.ReloadGroupRegionOptions(data) -- One aura selected
  else
    local triggerChoice = optionTriggerChoices[id]
    local trigger, untrigger =
      data.triggers[triggerChoice].trigger,
      data.triggers[triggerChoice].untrigger
    ...
    if triggerSystemOptionsFunction then
      trigger_options =
        union(
          trigger_options,
          triggerSystemOptionsFunction(data, optionTriggerChoices)
        )
    end
    if (trigger.type == "aura") then
      trigger_options =
    ...
    if childData and not regionTypes[childData.regionType] then
      ...
      if regionOptions[childData.regionType] then
        allOptions =
          union(
            allOptions,
            regionOptions[childData.regionType].create(id, data)
          )
      else
        ...
        allOptions["__unsupported" .. unsupportedCount] = {
          ...
          warning = {
            type = "description",
            name = L['Regions of type "%s" are not supported.']:format(
              childData.regionType
            ),
            order = 1
          }
        }
...
function WeakAuras.PositionOptions(
id,
  data,
  _,
  hideWidthHeight,
...
function WeakAuras.BorderOptions(
id,
  data,
  showBackDropOptions,
  hiddenFunc,
...
function WeakAuras.SortDisplayButtons(filter, overrideReset, id)
  ...
  for id, child in pairs(displayButtons) do
    ...
    if not data then
      ...
      if (frame.loadedButton:GetExpanded() and (not filter or id:lower():find(
        filter,
        1,
        true
      ) or containsFilter)) then
  ...
  for _, child in ipairs(to_sort) do
    ...
    if controlledChildren then
      table.sort(controlledChildren, function(a, b)
        return displayButtons[a]:GetGroupOrder() < displayButtons[b]:GetGroupOrder()
      end)
  ...
  for id, child in pairs(displayButtons) do
    ...
    if (frame.unloadedButton:GetExpanded() and (not filter or id:lower():find(
      filter,
      1,
      true
    ) or containsFilter)) then
  ...
  for id, child in pairs(displayButtons) do
    local group = child:GetGroup()
    local groupVisible =
      not group or visible[group] and displayButtons[group]:GetExpanded()
...
function WeakAuras.PickDisplay(
id,
  tab,
...
function WeakAuras.OpenModelPicker(data, field, parentData)
  if not IsAddOnLoaded("WeakAurasModelPaths") then
    ...
    if not loaded then
      ...
      print(
        WeakAuras.printPrefix .. "ModelPaths could not be loaded, the addon is " .. reason
      )
...
function WeakAuras.OpenTriggerTemplate(data, targetId)
  if not IsAddOnLoaded("WeakAurasTemplates") then
    ...
    if not loaded then
      reason = string.lower("|cffff2020" .. _G["ADDON_" .. reason] .. "|r.")
      print(
        WeakAuras.printPrefix .. "Templates could not be loaded, the addon is " .. reason
      )
      return
    end
...
function WeakAuras.ResetMoverSizer()
  if (frame and frame.mover and frame.moversizer and frame.mover.moving.region and frame.mover.moving.data) then
    frame.moversizer:SetToRegion(
      frame.mover.moving.region,
      frame.mover.moving.data
    )
  end
end
...
function WeakAuras.ShowCloneDialog(data)
  if (not (data.parent and WeakAuras.GetData(data.parent) and WeakAuras.GetData(
    data.parent
  ).regionType == "dynamicgroup") and not odb.preventCloneDialog) then
...
function WeakAuras.NewAura(sourceData, regionType, targetId)
  ...
  if (data.regionType ~= "group" and data.regionType ~= "dynamicgroup" and targetId) then
    ...
    if target then
      ...
      error(
        "Calling 'WeakAuras.NewAura' with invalid groupId. Reload your UI to fix the display list."
      )
    end
    -- move source into the top-level list
  else
...
function WeakAuras.SetCollapsed(id, namespace, path, v)
  ...
  if type(path) ~= "table" then
    collapsedOptions[id][namespace][path] =
      collapsedOptions[id][namespace][path] or {}
...
function WeakAuras.MoveCollapseDataUp(id, namespace, path)
  ...
  if type(path) ~= "table" then
    collapsedOptions[id][namespace][path],
      collapsedOptions[id][namespace][path - 1]
    =
      collapsedOptions[id][namespace][path - 1],
      collapsedOptions[id][namespace][path]
  else
