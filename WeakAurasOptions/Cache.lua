...
function spellCache.Build(callback)
  if cache then
    local co = coroutine.create(function() -- 136243 is the a gear icon, we can ignore those spells
      local id = 0
      local misses = 0
    ...
    error(
      "spellCache has not been loaded. Call WeakAuras.spellCache.Load(...) first."
    )
  end
end
