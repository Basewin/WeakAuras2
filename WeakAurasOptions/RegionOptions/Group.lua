...
local function createOptions(id, data)
  ...
  local options = {
    ...
    chooseIcon = {
      ...
      order = 0.51,
      func = function()
        WeakAuras.OpenIconPicker(data, "groupIcon", true)
      end
    },
    align_h = {
      type = "select",
      ...
      get = function()
        ...
        local alignedCenter, alignedRight, alignedLeft =
          "CENTER",
          "RIGHT",
          "LEFT"
          ...
          if (childData and childRegion) then
            if (v == "CENTER") then
              if childData.selfPoint:find("LEFT") then
                childData.xOffset =
                  0 - ((childData.width or childRegion.width) / 2)
    ...
    align_v = {
      ...
      get = function()
        ...
        local alignedCenter, alignedBottom, alignedTop =
          "CENTER",
          "RIGHT",
          "LEFT"
        for index, childId in pairs(data.controlledChildren) do
          ...
          if (childData and childRegion) then
            if (v == "CENTER") then
              if childData.selfPoint:find("BOTTOM") then
                childData.yOffset =
                  0 - ((childData.height or childRegion.height) / 2)
        ...
        for index, childId in pairs(data.controlledChildren) do
          ...
          if (childData and childRegion) then
            if (v > 0) then
              ...
              elseif childData.selfPoint:find("RIGHT") then
                childData.xOffset =
                  xOffset + (childData.width or childRegion.width)
              else
                childData.xOffset =
                  xOffset + ((childData.width or childRegion.width) / 2)
              end
              xOffset = xOffset + v
            elseif (v < 0) then
              if childData.selfPoint:find("LEFT") then
        ...
        for index, childId in pairs(data.controlledChildren) do
          ...
          if (childData and childRegion) then
            if (v > 0) then
              ...
              elseif childData.selfPoint:find("TOP") then
                childData.yOffset =
                  yOffset + (childData.height or childRegion.height)
              else
                childData.yOffset =
                  yOffset + ((childData.height or childRegion.height) / 2)
              end
              yOffset = yOffset + v
            elseif (v < 0) then
              if childData.selfPoint:find("BOTTOM") then
