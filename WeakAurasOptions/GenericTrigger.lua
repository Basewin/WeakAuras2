...
local function GetCustomTriggerOptions(data, optionTriggerChoices, trigger)
  ...
  local customOptions = {
    ...
    event_customError = {
      ...
      name = function()
        local events =
          trigger.custom_type == "event" and trigger.events2 or trigger.events
          ...
          for i in event:gmatch("[^:]+") do
            ...
            elseif trueEvent == "CLEU" or trueEvent == "COMBAT_LOG_EVENT_UNFILTERED" then
              ...
              if not WeakAuras.IsCLEUSubevent(subevent) then
                return "|cFFFF0000" .. L["%s is not a valid SubEvent for COMBAT_LOG_EVENT_UNFILTERED"]:format(
                  subevent
                )
              end
            elseif trueEvent:match("^UNIT_") then
              ...
              if not WeakAuras.baseUnitId[unit] and not WeakAuras.multiUnitId[unit] then
                return "|cFFFF0000" .. L["Unit %s is not a valid unit for RegisterUnitEvent"]:format(
                  unit
                )
              end
            end
  ...
  WeakAuras.AddCodeOption(
    ...
    L["Custom Variables"],
    "custom_variables",
    "https://github.com/WeakAuras/WeakAuras2/wiki/Trigger-State-Updater-(TSU)#custom-variables",
    11,
    hideCustomVariables,
    appendToTriggerPath("customVariables"),
    false,
    true,
    extraSetFunctionReload,
    nil,
    true
  )
    ...
    local function removeOverlay()
      if data.controlledChildren then
        for index, childId in ipairs(data.controlledChildren) do
          local childData = WeakAuras.GetData(childId)
          for j = i, 7 do
            childData.trigger["customOverlay" .. j] =
              childData.trigger["customOverlay" .. (j + 1)]
          end
...
local function GetGenericTriggerOptions(data, optionTriggerChoices)
  ...
  local options = {
    event = {
      ...
      hidden = function()
        return not (trigger.type == "event" or trigger.type == "status")
      end
    }
  }
  ...
  local combatLogOptions = {
    subeventPrefix = {
      ...
      control = "WeakAurasSortedDropdown",
      hidden = function()
        return not (trigger.type == "event" and trigger.event == "Combat Log")
      end
    },
    subeventSuffix = {
      type = "select",
      ...
      control = "WeakAurasSortedDropdown",
      hidden = function()
        return not (trigger.type == "event" and trigger.event == "Combat Log" and subevent_actual_prefix_types[trigger.subeventPrefix])
      end
    },
    spacer_suffix = {
      type = "description",
  ...
  elseif (triggerType == "status" or triggerType == "event") then
    ...
    if data.controlledChildren then
      ...
      if (event and WeakAuras.event_prototypes[event]) then
        prototypeOptions =
          WeakAuras.ConstructOptions(
            WeakAuras.event_prototypes[event],
            data,
            10,
      ...
      if WeakAuras.event_prototypes[trigger.event] then
        prototypeOptions =
          WeakAuras.ConstructOptions(
            WeakAuras.event_prototypes[trigger.event],
            data,
            10,
            optionTriggerChoices[id]
          )
...
WeakAuras.RegisterTriggerSystemOptions(
  { "event", "status", "custom" },
  GetGenericTriggerOptions
)