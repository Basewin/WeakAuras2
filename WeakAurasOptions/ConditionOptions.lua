    ...
    if (object["same" .. variable] == false) then
      ...
      for id, reference in pairs(object.references) do
        if (type == "list" and values) then
          desc =
            desc .. "|cFFE0E000" .. id .. ": |r" .. (values[reference[variable]] or "") .. "\n"
        else
          desc =
            desc .. "|cFFE0E000" .. id .. ": |r" .. (valueToString(
              reference[variable],
              type
            ) or "") .. "\n"
        end
  ...
  if data.controlledChildren then
    ...
    if (object["same" .. variable] and object["same" .. variable][subvariable] == false) then
      ...
      for id, reference in pairs(object.references) do
        if values then
          desc =
            desc .. "|cFFE0E000" .. id .. ": |r" .. (values[reference[variable][subvariable]] or "") .. "\n"
        else
          desc =
            desc .. "|cFFE0E000" .. id .. ": |r" .. (valueToString(
              reference[variable][subvariable],
              type,
              subvariable
            ) or "") .. "\n"
        end
...
local function filterUsedProperties(
indexToProperty,
  allDisplays,
  usedProperties,
  ownProperty
...
local function addControlsForChange(
args,
  order,
  data,
  conditionVariable,
  ...
  local display =
    isSubset(
      data,
      conditions[i].changes[j]
    ) and allProperties.displayWithCopy or allProperties.display
  local valuesForProperty =
    filterUsedProperties(
      allProperties.indexToProperty,
      display,
      usedProperties,
      conditions[i].changes[j].property
    )
    ...
    set = function(info, index)
        ...
        for _, id in ipairs(data.controlledChildren) do
          if conditions[i].changes[j].references[id] then
            ...
            local conditionIndex =
              conditions[i].check.references[id].conditionIndex
            local auraData = WeakAuras.GetData(id)
            tinsert(
              auraData[conditionVariable][conditionIndex].changes,
              insertPoint,
              change
            )
      ...
      if data.controlledChildren then
        for id, reference in pairs(conditions[i].changes[j].references) do
          ...
          local conditionIndex =
            conditions[i].check.references[id].conditionIndex
          auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].property
          = property
  ...
  if data.controlledChildren then
    ...
    setValueColor = function(info, r, g, b, a)
      for id, reference in pairs(conditions[i].changes[j].references) do
        ...
        auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value
        =
          auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value or {}
        auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value[1]
        = r
        auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value[2]
        = g
        auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value[3]
        = b
        auraData[conditionVariable][conditionIndex].changes[reference.changeIndex].value[4]
        = a
        WeakAuras.Add(auraData)
      end
  ...
  if property then
    propertyType =
      allProperties.propertyMap[property] and allProperties.propertyMap[property].type
  end
  if (propertyType == "bool" or propertyType == "number") then
    ...
    if (propertyType == "number") then
      ...
      if (properties.min or properties.softMin) and (properties.max or properties.softMax) then
        ...
        args["condition" .. i .. "value" .. j].validate =
          WeakAuras.ValidateNumeric
      end
    end
  elseif (propertyType == "color") then
    args["condition" .. i .. "value" .. j] = {
      ...
      hasAlpha = true,
      get = function()
        if (conditions[i].changes[j].value and type(
          conditions[i].changes[j].value
        ) == "table") then
  ...
  elseif (propertyType == "list") then
    local values =
      property and allProperties.propertyMap[property] and allProperties.propertyMap[property].values
    args["condition" .. i .. "value" .. j] = {
      type = "select",
      width = WeakAuras.normalWidth,
      values = values,
  ...
  elseif (propertyType == "sound") then
    args["condition" .. i .. "value" .. j .. "sound_type"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound_type
      end,
    ...
    local function anySoundType(needle)
      ...
      if conditions[i].changes[j].references then
        for id, reference in pairs(conditions[i].changes[j].references) do
          if (type(
            reference.value
          ) == "table" and reference.value.sound_type == needle) then
            return true
          end
        end
      end
    ...
    args["condition" .. i .. "value" .. j .. "sound"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound
      end,
    ...
    args["condition" .. i .. "value" .. j .. "sound_channel"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound_channel
      end,
    ...
    args["condition" .. i .. "value" .. j .. "sound_repeat"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound_repeat
      end,
    ...
    local function anySoundValue(needle)
      ...
      if conditions[i].changes[j].references then
        for id, reference in pairs(conditions[i].changes[j].references) do
          if (type(
            reference.value
          ) == "table" and reference.value.sound == needle) then
            return true
          end
        end
      end
    ...
    args["condition" .. i .. "value" .. j .. "sound_path"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound_path
      end,
      set = wrapWithPlaySound(setValueComplex("sound_path")),
      hidden = function()
        return not (anySoundValue(" custom") and (anySoundType(
          "Loop"
        ) or anySoundType("Play")))
      end
    }
    order = order + 1
    ...
    args["condition" .. i .. "value" .. j .. "sound_kit_id"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.sound_kit_id
      end,
      set = wrapWithPlaySound(setValueComplex("sound_kit_id")),
      hidden = function()
        return not (anySoundValue(" KitID") and (anySoundType(
          "Loop"
        ) or anySoundType("Play")))
      end
    }
  ...
  elseif (propertyType == "chat") then
    args["condition" .. i .. "value" .. j .. "message type"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.message_type
      end,
    ...
    local function anyMessageType(needle)
      ...
      if conditions[i].changes[j].references then
        for id, reference in pairs(conditions[i].changes[j].references) do
          if (type(
            reference.value
          ) == "table" and reference.value.message_type == needle) then
            return true
          end
        end
      end
    ...
    if not WeakAuras.IsClassic() then
      args["condition" .. i .. "value" .. j .. "message type warning"] = {
        ...
        order = order,
        hidden = function()
          return not (anyMessageType("SAY") or anyMessageType(
            "YELL"
          ) or anyMessageType("SMARTRAID"))
        end
      }
      order = order + 1
    end
    ...
    args["condition" .. i .. "value" .. j .. "message dest"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.message_dest
      end,
    ...
    args["condition" .. i .. "value" .. j .. "message channel"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.message_channel
      end,
    ...
    if (not descMessage and data ~= WeakAuras.tempGroup) then
      descMessage =
        L["Dynamic text tooltip"] .. WeakAuras.GetAdditionalProperties(data)
    end
    ...
    args["condition" .. i .. "value" .. j .. "message"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.message
      end,
    ...
    args["condition" .. i .. "value" .. j .. "custom"] = {
      ...
      get = function()
        return type(
          conditions[i].changes[j].value
        ) == "table" and conditions[i].changes[j].value.custom
      end,
  ...
  elseif (propertyType == "customcode") then
          ...
          func = function()
            if data.controlledChildren then
              ...
              for id, reference in pairs(conditions[i].changes[j].references) do
                ...
                childData.conditions[conditionIndex].changes[changeIndex].value
                =
                  childData.conditions[conditionIndex].changes[changeIndex].value or {}
                multipath[id] =
                  {
                    "conditions",
                    conditionIndex,
              ...
              data.conditions[i].changes[j].value =
                data.conditions[i].changes[j].value or {}
              WeakAuras.OpenTextEditor(
                data,
                { "conditions", i, "changes", j, "value", "custom" },
                true
    ...
    args["condition" .. i .. "value" .. j .. "custom_error"] = {
      ...
      name = function()
        ...
        local _, errorString =
          loadstring("return function() " .. custom .. "\n end")
        return errorString and "|cFFFF0000" .. errorString or ""
      end,
      width = WeakAuras.doubleWidth,
      order = order,
...
args,
  ...
  local conditionTemplatesToUse =
    indentDepth < 3 and conditionTemplates or conditionTemplateWithoutCombinations

  local optionsName = blueIfSubset(data, conditions[i].check)
  local needsTriggerName =
    check and check.trigger and check.trigger ~= -1 and check.trigger ~= -2
  if parentType then
    local isFirst = path[#path] == 1
    if isFirst then
      if needsTriggerName then
        optionsName =
          optionsName .. string.format(L["Trigger %s"], check.trigger)
      end
    else
      if needsTriggerName then
        if (parentType == "AND") then
          optionsName =
            optionsName .. string.format(L["and Trigger %s"], check.trigger)
        else
          optionsName =
            optionsName .. string.format(L["or Trigger %s"], check.trigger)
        end
      end
    end
  else
    if needsTriggerName then
      optionsName =
        optionsName .. string.format(L["If Trigger %s"], check.trigger)
    else
  ...
  if (indentDepth > 0) then
    ...
    valuesForIf =
      isSubset(
        data,
        conditions[i].check
      ) and conditionTemplatesToUse.displayWithCopy or conditionTemplatesToUse.display
  end
  ...
  args["condition" .. i .. tostring(path) .. "if"] = {
    ...
    set = function(info, v)
        ...
        for _, id in ipairs(data.controlledChildren) do
          if conditions[i].check.references[id] then
            ...
            for changeIndex, change in ipairs(conditions[i].changes) do
              local propertyType =
                change.property and allProperties.propertyMap[change.property] and allProperties.propertyMap[change.property].type
      ...
      if (conditionTemplatesToUse.indexToTrigger[v] == "REMOVE") then
        if data.controlledChildren then
          for id, reference in pairs(conditions[i].check.references) do
            local auraData = WeakAuras.GetData(id)
            removeSubCheck(
              auraData[conditionVariable][reference.conditionIndex].check,
              path
            )
          end
        else
      ...
      if (not trigger or not variable) then return end
      ...
      if data.controlledChildren then
        ...
        if (check.trigger and check.variable) then
          ...
          local templatesForTriggerAndCondition =
            templatesForTrigger and templatesForTrigger[check.variable]
          oldType =
            templatesForTriggerAndCondition and templatesForTriggerAndCondition.type
        end
  ...
  if (currentConditionTemplate and currentConditionTemplate.type and type(
    currentConditionTemplate.type
  ) == "string") then
    local setOp
    local setValue
    if data.controlledChildren then
...
local function addControlsForCondition(
args,
  order,
  data,
  conditionVariable,
  ...
  if not conditions[i].check then return end
  ...
  args["condition" .. i .. "_addChange"] = {
    ...
    func = function()
      if data.controlledChildren then
        for id, reference in pairs(conditions[i].check.references) do
          local auradata = WeakAuras.GetData(id)
          auradata[conditionVariable][reference.conditionIndex].changes =
            auradata[conditionVariable][reference.conditionIndex].changes or {}
          tinsert(
            auradata[conditionVariable][reference.conditionIndex].changes,
            {}
          )
...
local function mergeConditionTemplates(
allConditionTemplates,
  auraConditionsTemplate,
  numTriggers
)
  for triggernum = 1, numTriggers do
    ...
    if auraTemplatesForTrigger then
      allConditionTemplates[triggernum] =
        allConditionTemplates[triggernum] or {}
      for conditionName in pairs(auraTemplatesForTrigger) do
        if not allConditionTemplates[triggernum][conditionName] then
          ...
          WeakAuras.DeepCopy(
            auraTemplatesForTrigger[conditionName],
            allConditionTemplates[triggernum][conditionName]
          )
          ...
          if (allConditionTemplates[triggernum][conditionName].type ~= auraTemplatesForTrigger[conditionName].type) then
            allConditionTemplates[triggernum][conditionName].type =
              "incompatible"
          end
        end
      end
...
local function createConditionTemplatesValueList(
allConditionTemplates,
  numTriggers,
  excludeCombinations
)
      ...
      if templatesForTrigger then
        ...
        if (#sorted > 0) then
          ...
          elseif (triggernum == -1) then
            conditionTemplates.display[index] =
              string.format(L["Global Conditions"])
          else
            conditionTemplates.display[index] =
              string.format(L["Trigger %d"], triggernum)
          end
          index = index + 1

          for _, conditionName in ipairs(sorted) do
            conditionTemplates.display[index] =
              "    " .. templatesForTrigger[conditionName].display
            conditionTemplates.indexToTrigger[index] = triggernum
            conditionTemplates.indexToVariable[index] = conditionName
            conditionTemplates.conditionToIndex[triggernum .. "-" .. conditionName]
            = index
            index = index + 1
          end
        end
...
local function createConditionTemplates(data)
  ...
  local conditionTemplates =
    createConditionTemplatesValueList(allConditionTemplates, numTriggers)
...
local function compareSubChecks(a, b, allConditionTemplates)
  ...
  for i = 1, count do
    ...
    if (a[i].trigger == -2) then
      ...
      local currentConditionTemplate =
        allConditionTemplates[a[i].trigger] and allConditionTemplates[a[i].trigger][a[i].variable]
  ...
  for index, allChange in ipairs(all) do
    if (allChange.property == change.property) then
      local alreadyReferenced =
        allChange.references and allChange.references[id]
...
local function mergeConditionChange(all, change, id, changeIndex, allProperties)
  local propertyType =
    all.property and allProperties.propertyMap[all.property] and allProperties.propertyMap[all.property].type
    ...
    if (type(all.value) ~= type(change.value)) then
      ...
      if (type(change.value) ~= "table") then
        ...
        for _, propertyName in
          ipairs(propertyTypeToSubProperty[propertyType])
        do
...
local function mergeConditions(
all,
  aura,
  id,
  allConditionTemplates,
    ...
    if not match then
      ...
      if copy.changes then
        for changeIndex, change in pairs(copy.changes) do
          local propertyType =
            change.property and propertyTypes.propertyMap[change.property] and propertyTypes.propertyMap[change.property].type
          if (propertyType == "chat" or propertyType == "sound" or propertyType == "customcode") then
            change.samevalue = {}
            for _, propertyName in
              ipairs(propertyTypeToSubProperty[propertyType])
...
function WeakAuras.GetConditionOptions(
data,
  args,
  conditionVariable,
  startorder,
  ...
  local conditionTemplates, conditionTemplateWithoutCombinations =
    createConditionTemplates(data)
