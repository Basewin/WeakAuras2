...
function TestForLongString(trigger, arg)
  ...
  if (trigger[name .. "_operator"] == "==") then
    ...
    test =
      "(" .. name .. " and " .. name .. ":" .. trigger[name .. "_operator"]:format(
        trigger[name]
      ) .. ")"
  end
  return test
...
function TestForMultiSelect(trigger, arg)
  ...
  elseif trigger["use_" .. name] then
    ...
    if not arg.test then
      test =
        trigger[name].single and "(" .. name .. "==" .. (tonumber(
          value
        ) or "[[" .. value .. "]]") .. ")"
    else
      test =
        trigger[name].single and "(" .. arg.test:format(
          tonumber(value) or "[[" .. value .. "]]"
        ) .. ")"
    end
  end
...
function ConstructTest(trigger, arg)
  ...
  if (arg.hidden or arg.type == "tristate" or arg.type == "toggle" or (arg.type == "multiselect" and trigger["use_" .. name] ~= nil) or ((trigger["use_" .. name] or arg.required) and trigger[name])) then
    ...
    elseif (arg.type == "string" or type == "select" or type == "spell" or type == "item") then
      test =
        "(" .. name .. " and " .. name .. "==" .. (number or '"' .. (trigger[name] or "") .. '"') .. ")"
    else
      ...
      -- number
      test =
        "(" .. name .. " and " .. name .. (trigger[name .. "_operator"] or "==") .. (number or '"' .. (trigger[name] or "") .. '"') .. ")"
    end
  end
...
local function RunOverlayFuncs(event, state)
  ...
  for i, overlayFunc in ipairs(event.overlayFuncs) do
    ...
    local ok, a, b, c =
      xpcall(overlayFunc, geterrorhandler(), event.trigger, state)
    if not ok then
...
local function callFunctionForActivateEvent(
func,
  trigger,
  fallback,
  errorHandler
...
function WeakAuras.ActivateEvent(id, triggernum, data, state, errorHandler)
  ...
  elseif data.durationFunc then
    local ok, arg1, arg2, arg3, inverse =
      xpcall(data.durationFunc, errorHandler, data.trigger)
  ...
  if data.triggerFunc then
    ...
    if (untriggerCheck and not optionsEvent) then
      ...
      elseif (data.statesParameter == "one") then
        ...
        if data.untriggerFunc then
          local ok, returnValue =
            xpcall(data.untriggerFunc, errorHandler, event, arg1, arg2, ...)
          if (ok and returnValue) then
            allStates[""] = allStates[""] or {}
            local state = allStates[""]
            if WeakAuras.EndEvent(id, triggernum, nil, state) then
...
local function AddFakeTime(state)
  if state.progressType == "timed" then
    if state.expirationTime and state.expirationTime ~= math.huge and state.expirationTime > GetTime() then return end
    state.progressType = "timed"
...
function HandleEvent(frame, event, arg1, arg2, ...)
  ...
  if (event == "PLAYER_ENTERING_WORLD") then
    timer:ScheduleTimer(function()
      WeakAuras.StartProfileSystem(
        "generictrigger WA_DELAYED_PLAYER_ENTERING_WORLD"
      )
    ...
    for index, event in pairs(data.events) do
      ...
      if (event == "COMBAT_LOG_EVENT_UNFILTERED" and data.subevents) then
        for i, subevent in pairs(data.subevents) do
          ...
          loaded_events[event][subevent][id] =
            loaded_events[event][subevent][id] or {}
  ...
  if data.unit_events then
    for unit, events in pairs(data.unit_events) do
      ...
      for index, event in pairs(events) do
        MultiUnitLoop(function(unit)
          loaded_unit_events[unit] = loaded_unit_events[unit] or {}
          loaded_unit_events[unit][event] =
            loaded_unit_events[unit][event] or {}
          loaded_unit_events[unit][event][id] =
            loaded_unit_events[unit][event][id] or {}
...
function GenericTrigger.LoadDisplays(toLoad, loadEvent, ...)
  ...
  for unit, events in pairs(unitEventsToRegister) do
    for event in pairs(events) do
      ...
      xpcall(
        frame.unitFrames[unit].RegisterUnitEvent,
        trueFunction,
        frame.unitFrames[unit],
        event,
        unit
      )
      genericTriggerRegisteredUnitEvents[unit] =
        genericTriggerRegisteredUnitEvents[unit] or {}
      ...
      if (triggerType == "status" or triggerType == "event" or triggerType == "custom") then
        ...
        if (triggerType == "status" or triggerType == "event") then
          if not trigger.event then
            error(
              'Improper arguments to WeakAuras.Add - trigger type is "event" but event is not defined'
            )
          elseif not event_prototypes[trigger.event] then
            if event_prototypes["Health"] then
              ...
              error(
                'Improper arguments to WeakAuras.Add - no event prototype can be found for event type "' .. trigger.event .. '" and default prototype reset failed.'
              )
            end
          else
            ...
            triggerFuncStr =
              ConstructFunction(event_prototypes[trigger.event], trigger)
            ...
            if event_prototypes[trigger.event].overlayFuncs then
              ...
              local dest = 1
              for i, v in
                ipairs(event_prototypes[trigger.event].overlayFuncs)
              do
            ...
            elseif event_prototypes[trigger.event].timedrequired then
              if type(
                event_prototypes[trigger.event].timedrequired
              ) == "function" then
            ...
            if (trigger.unevent == "custom") then
              untriggerFuncStr =
                ConstructFunction(event_prototypes[trigger.event], untrigger)
          ...
          triggerFunc =
            WeakAuras.LoadFunction("return " .. (trigger.custom or ""), id)
          if (trigger.custom_type == "stateupdate") then
            tsuConditionVariables =
          ...
          if (trigger.custom_type == "status" or trigger.custom_type == "event" and trigger.custom_hide == "custom") then
            untriggerFunc =
              WeakAuras.LoadFunction("return " .. (untrigger.custom or ""), id)
          ...
          if (trigger.custom_type ~= "stateupdate" and trigger.customDuration and trigger.customDuration ~= "") then
            durationFunc =
              WeakAuras.LoadFunction("return " .. trigger.customDuration, id)
          end
          if (trigger.custom_type ~= "stateupdate") then
            overlayFuncs = {}
            for i = 1, 7 do
            ...
            for index, event in pairs(rawEvents) do
              ...
              for i in event:gmatch("[^:]+") do
                if not trueEvent then
                  ...
                  isCLEU =
                    trueEvent == "CLEU" or trueEvent == "COMBAT_LOG_EVENT_UNFILTERED"
                elseif isCLEU then
              ...
              force_events =
                trigger.custom_type == "status" or trigger.custom_type == "stateupdate"
            end
          end
          if (trigger.custom_type == "stateupdate") then
...
do
  ...
  function WeakAuras.UnregisterAllEveryFrameUpdate()
    if not update_frame then return end
    wipe(update_clients)
  ...
  for triggernum, triggerData in ipairs(data.triggers) do
    ...
    if (data.internalVersion < 2) then
      ...
      if (trigger and trigger.custom) then
        trigger.custom =
          trigger.custom:gsub(
            "COMBAT_LOG_EVENT_UNFILTERED_CUSTOM",
      ...
      if (trigger and trigger.type and trigger.event and trigger.type == "status" and (trigger.event == "Cooldown Progress (Spell)" or trigger.event == "Cooldown Progress (Item)" or trigger.event == "Death Knight Rune")) then
...
do
  ...
  local function swingTimerCLEUCheck(
  ts,
    event,
    _,
    sourceGUID,
    ...
    elseif (destGUID == selfGUID and (select(1, ...) == "PARRY" or select(
      4,
      ...
    ) == "PARRY")) then
      if lastSwingMain then
        ...
        if (timeLeft > 0.6 * swingDurationMain) then
          ...
          mainTimer =
            timer:ScheduleTimerFixed(
              swingEnd,
              timeLeft - 0.4 * swingDurationMain,
              "main"
            )
        ...
        elseif (timeLeft > 0.2 * swingDurationMain) then
          ...
          mainTimer =
            timer:ScheduleTimerFixed(
              swingEnd,
              timeLeft - 0.2 * swingDurationMain,
              "main"
            )
    ...
    if event == "UNIT_ATTACK_SPEED" then
      ...
      if lastSwingMain then
        if mainSpeedNew ~= mainSpeed then
          ...
          local timeLeft =
            (lastSwingMain + swingDurationMain - GetTime()) * multiplier
      ...
      if lastSwingOff then
        if offSpeedNew ~= offSpeed then
          ...
          local timeLeft =
            (lastSwingOff + swingDurationOff - GetTime()) * multiplier
...
do
  ...
  local RecheckHandles = {
    ...
    Schedule = function(self, expirationTime, id)
      if (not self.expirationTime[id] or expirationTime < self.expirationTime[id]) and expirationTime > 0 then
        ...
        if duration > 0 then
          self.handles[id] =
            timer:ScheduleTimerFixed(self.Recheck, duration, self, id)
  ...
  local function HandleSpell(self, id, startTime, duration)
    ...
    if duration > 0 then
      if (startTime == gcdStart and duration == gcdDuration) or (WeakAuras.IsClassic() and duration == shootDuration and startTime == shootStart) then
  ...
  function WeakAuras.InitCooldownReady()
    ...
    cdReadyFrame:SetScript("OnEvent", function(self, event, ...)
      ...
      if (event == "SPELL_UPDATE_COOLDOWN" or event == "SPELL_UPDATE_CHARGES" or event == "RUNE_POWER_UPDATE" or event == "ACTIONBAR_UPDATE_COOLDOWN" or event == "PLAYER_TALENT_UPDATE" or event == "PLAYER_PVP_TALENT_UPDATE") then
        WeakAuras.CheckCooldownReady()
  ...
  function WeakAuras.GetSpellCooldown(
  id,
    ignoreRuneCD,
    showgcd,
    ignoreSpellKnown,
    ...
    if (not spellKnown[id] and not ignoreSpellKnown) then return end
  ...
  function WeakAuras.GetItemSlotCooldown(id, showgcd)
    ...
    if (itemSlots[id] and itemSlotsCdExps[id] and itemSlotsCdDurs[id]) then
      startTime, duration, enabled =
        itemSlotsCdExps[id] - itemSlotsCdDurs[id],
        itemSlotsCdDurs[id],
        itemSlotsEnable[id]
    else
    ...
    for id, _ in pairs(runes) do
      ...
      if (duration > 0 and duration ~= WeakAuras.gcdDuration()) then
        ...
        if not runeCdExps[id] then
          ...
          runeCdHandles[id] =
            timer:ScheduleTimerFixed(RuneCooldownFinished, endTime - time, id)
  ...
  function WeakAuras.GetSpellCooldownUnified(id, runeDuration)
    ...
    local onNonGCDCD =
      durationCooldown and startTimeCooldown and durationCooldown > 0 and (durationCooldown ~= gcdDuration or startTimeCooldown ~= gcdStart)
    if onNonGCDCD then
      cooldownBecauseRune =
        runeDuration and durationCooldown and abs(
          durationCooldown - runeDuration
        ) < 0.001
    ...
    return charges, maxCharges, startTime, duration, unifiedCooldownBecauseRune, startTimeCooldown, durationCooldown, cooldownBecauseRune, startTimeCharges, durationCharges, GetSpellCount(
      id
    )
  end
  ...
  function WeakAuras.CheckSpellCooldown(id, runeDuration)
    ...
    local chargesChanged =
      spellCharges[id] ~= charges or spellCounts[id] ~= spellCount
    local chargesDifference =
      (charges or spellCount or 0) - (spellCharges[id] or spellCount or 0)
    ...
    if not cooldownBecauseRune then
      changed =
        spellCdsOnlyCooldownRune:HandleSpell(
          id,
          startTimeCooldown,
          durationCooldown
        ) or changed
    end
    local chargeChanged, chargeNowReady =
    ...
    for id, _ in pairs(items) do
      ...
      if (duration > 0 and duration > 1.5 and duration ~= WeakAuras.gcdDuration()) then
        ...
        if not itemCdExps[id] then
          ...
          itemCdHandles[id] =
            timer:ScheduleTimerFixed(ItemCooldownFinished, endTime - time, id)
    ...
    for id, itemId in pairs(itemSlots) do
      ...
      if (duration > 0 and duration > 1.5 and duration ~= WeakAuras.gcdDuration()) then
        ...
        if not itemSlotsCdExps[id] then
          ...
          itemSlotsCdHandles[id] =
            timer:ScheduleTimerFixed(
              ItemSlotCooldownFinished,
              endTime - time,
              id
            )
  ...
  local function InitSpellActivation()
    ...
    spellActivationFrame:SetScript("OnEvent", function(self, event, spell)
      ...
      if spellActivationSpells[spell] then
        spellActivationSpellsCurrent[spell] =
...
function WeakAuras.GetEquipmentSetInfo(itemSetName, partial)
  ...
  for index, id in pairs(equipmentSetIds) do
    local name, icon, _, _, numItems, numEquipped =
      C_EquipmentSet.GetEquipmentSetInfo(id)
    if (itemSetName == nil or (name and itemSetName == name)) then
      if (name ~= nil) then
        local match =
          (not partial and numItems == numEquipped) or (partial and (numEquipped or 0) > bestMatchNumEquipped)
        if match then
...
do
  ...
  local function dbmRecheckTimers()
    ...
    if nextExpire then
      recheckTimer =
        timer:ScheduleTimerFixed(dbmRecheckTimers, nextExpire - now)
    end
  end
  ...
  local function dbmEventCallback(event, ...)
    if event == "DBM_TimerStart" then
      ...
      if dbmType == 1 then
        r, g, b =
          barOptions.StartColorAR,
          barOptions.StartColorAG,
          barOptions.StartColorAB
      ...
      elseif dbmType == 3 then
        r, g, b =
          barOptions.StartColorDR,
          barOptions.StartColorDG,
          barOptions.StartColorDB
      elseif dbmType == 4 then
        r, g, b =
          barOptions.StartColorIR,
          barOptions.StartColorIG,
          barOptions.StartColorIB
      elseif dbmType == 5 then
        r, g, b =
          barOptions.StartColorRR,
          barOptions.StartColorRG,
          barOptions.StartColorRB
      elseif dbmType == 6 then
        r, g, b =
          barOptions.StartColorPR,
          barOptions.StartColorPG,
          barOptions.StartColorPB
      elseif dbmType == 7 then
        ...
        r, g, b =
          barOptions.StartColorR,
          barOptions.StartColorG,
          barOptions.StartColorB
      end
      bar.dbmColor = { r, g, b }
      ...
      if nextExpire == nil then
        recheckTimer =
          timer:ScheduleTimerFixed(dbmRecheckTimers, expirationTime - now)
    ...
    elseif event == "DBM_TimerUpdate" then
      ...
      if bar then
        ...
        if expirationTime < nextExpire then
          timer:CancelTimer(recheckTimer)
          recheckTimer =
            timer:ScheduleTimerFixed(dbmRecheckTimers, duration - elapsed)
          nextExpire = expirationTime
        end
      end
  ...
  function WeakAuras.DBMTimerMatches(
  timerId,
    id,
    message,
    operator,
  ...
  function WeakAuras.GetDBMTimer(
  id,
    message,
    operator,
    spellId,
    ...
    for timerId, bar in pairs(bars) do
      if WeakAuras.DBMTimerMatches(
        ...
        dbmType,
        count
      ) and (bestMatch == nil or bar.expirationTime < bestMatch.expirationTime) and bar.expirationTime + extendTimer > GetTime() then
        bestMatch = bar
      end
    end
    return bestMatch
  ...
  function WeakAuras.RegisterDBMCallback(event)
    if registeredDBMEvents[event] then return end
    if DBM then
  ...
  local function doDbmScan(fireTime)
    WeakAuras.debug(
      "Performing dbm scan at " .. fireTime .. " (" .. GetTime() .. ")"
    )
  ...
  function WeakAuras.ScheduleDbmCheck(fireTime)
    if not scheduled_scans[fireTime] then
      scheduled_scans[fireTime] =
        timer:ScheduleTimerFixed(
          doDbmScan,
          fireTime - GetTime() + 0.1,
          fireTime
        )
...
do
  ...
  local function bigWigsEventCallback(event, ...)
    ...
    elseif event == "BigWigs_StartBar" then
      ...
      bar.bwBackgroundColor =
        BWColorModule:GetColorTable("barBackground", addon, spellId)
      local BWEmphasizedModule = BigWigs:GetPlugin("Super Emphasize")
      bar.emphasized =
        BWEmphasizedModule:IsSuperEmphasized(addon, spellId) and true or false
      ...
      if nextExpire == nil then
        recheckTimer =
          timer:ScheduleTimerFixed(recheckTimers, expirationTime - now)
        nextExpire = expirationTime
      elseif expirationTime < nextExpire then
        timer:CancelTimer(recheckTimer)
        recheckTimer =
  ...
  function WeakAuras.RegisterBigWigsCallback(event)
    if registeredBigWigsEvents[event] then return end
  ...
  function WeakAuras.BigWigsTimerMatches(
  id,
    message,
    operator,
    spellId,
  ...
  function WeakAuras.GetBigWigsTimer(
  text,
    operator,
    spellId,
    extendTimer,
    ...
    local bestMatch
    for id, bar in pairs(bars) do
      if WeakAuras.BigWigsTimerMatches(
        id,
        text,
        operator,
        spellId,
  ...
  local function doBigWigsScan(fireTime)
    WeakAuras.debug(
      "Performing BigWigs scan at " .. fireTime .. " (" .. GetTime() .. ")"
    )
    scheduled_scans[fireTime] = nil
...
function WeakAuras.CheckTotemName(
totemName,
  triggerTotemName,
  triggerTotemPattern,
  triggerTotemOperator
)
...
do
  ...
  local function doCooldownScan(fireTime)
    WeakAuras.debug(
      "Performing cooldown scan at " .. fireTime .. " (" .. GetTime() .. ")"
    )
    scheduled_scans[fireTime] = nil
...
do
  ...
  local function doCastScan(firetime, unit)
    WeakAuras.debug(
      "Performing cast scan at " .. firetime .. " (" .. GetTime() .. ")"
    )
    scheduled_scans[firetime] = nil
  ...
  if (trigger.type == "event" or trigger.type == "status") then
    if trigger.event and WeakAuras.event_prototypes[trigger.event] then
      if WeakAuras.event_prototypes[trigger.event].durationFunc then
        if (type(
          WeakAuras.event_prototypes[trigger.event].init
        ) == "function") then
          WeakAuras.event_prototypes[trigger.event].init(trigger)
        end
        local current, maximum, custom =
          WeakAuras.event_prototypes[trigger.event].durationFunc(trigger)
...
function GenericTrigger.GetOverlayInfo(data, triggernum)
  ...
  if (trigger.type ~= "custom" and trigger.event and WeakAuras.event_prototypes[trigger.event] and WeakAuras.event_prototypes[trigger.event].overlayFuncs) then
    ...
    local dest = 1
    for i, v in
      ipairs(WeakAuras.event_prototypes[trigger.event].overlayFuncs)
    do
      if v.enable(trigger) then
        result[dest] = v.name
        dest = dest + 1
    ...
    if (trigger.event and WeakAuras.event_prototypes[trigger.event]) then
      ...
      elseif WeakAuras.event_prototypes[trigger.event].hasItemID then
        GameTooltip:SetHyperlink(
          "item:" .. trigger.itemName .. ":0:0:0:0:0:0:0"
        )
        return true
      end
    end
    ...
    if (trigger.event and WeakAuras.event_prototypes[trigger.event]) then
      ...
      for _, v in pairs(WeakAuras.event_prototypes[trigger.event].args) do
        ...
        if (enable and v.store and v.name and v.display) then
          found = true
          additional =
            additional .. "|cFFFF0000%" .. triggernum .. "." .. v.name .. "|r - " .. v.display .. "\n"
        end
      end
...
function GenericTrigger.GetTriggerConditions(data, triggernum)
  ...
  elseif (trigger.type == "custom") then
    if (trigger.custom_type == "status" or trigger.custom_type == "event") then
      ...
      local canHaveDurationFunc =
        trigger.custom_type == "status" or (trigger.custom_type == "event" and (trigger.custom_hide ~= "timed" or trigger.dynamicDuration))
    ...
    elseif (trigger.custom_type == "stateupdate") then
      if (events[data.id][triggernum] and events[data.id][triggernum].tsuConditionVariables) then
        if type(
          events[data.id][triggernum].tsuConditionVariables
        ) ~= "table" then
          return nil
        end
        local result =
          CopyTable(events[data.id][triggernum].tsuConditionVariables)
        if not result then
          return nil
        end
...
function GenericTrigger.CreateFallbackState(data, triggernum, state)
  ...
  if event.textureFunc then
    local ok, texture =
      xpcall(event.textureFunc, geterrorhandler(), firstTrigger)
  ...
  if event.durationFunc then
    local ok, arg1, arg2, arg3, inverse =
      xpcall(event.durationFunc, geterrorhandler(), firstTrigger)
    if not ok then
      state.progressType = "timed"
      state.duration = 0
  ...
  if (trigger.type == "event" or trigger.type == "status") then
    if (trigger.type == "event") then
      tinsert(
        ...
        {
          L["Trigger:"],
          (WeakAuras.event_types[trigger.event] or L["Undefined"])
        }
      )
    else
      tinsert(
        ...
        {
          L["Trigger:"],
          (WeakAuras.status_types[trigger.event] or L["Undefined"])
        }
      )
    end
    if (trigger.event == "Combat Log" and trigger.subeventPrefix and trigger.subeventSuffix) then
      tinsert(
        ...
        {
          L["Message type:"],
          (WeakAuras.subevent_prefix_types[trigger.subeventPrefix] or L["Undefined"]) .. " " .. (WeakAuras.subevent_suffix_types[trigger.subeventSuffix] or L["Undefined"])
        }
      )
    end
  else
