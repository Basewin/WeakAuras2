...
do
  ...
  local function TestNonUniformSettings(acEntry, data)
    ...
    -- Test OwnOnly
    if (data.ownOnly == true and WeakAuras.myGUID ~= acEntry.casterGUID or data.ownOnly == false and WeakAuras.myGUID == acEntry.casterGUID) then
      return false
    end
  ...
  function aura_cache.Watch(self, id, triggernum)
    ...
    self.watched[id][triggernum].players =
      self.watched[id][triggernum].players or {}
    self:ForceUpdate()
  end
  ...
  function aura_cache.GetNumber(self, id, triggernum, data)
    ...
    for guid, _ in pairs(self.players) do
      ...
      if (self.watched[id][triggernum].players[guid] and TestNonUniformSettings(
  ...
  function aura_cache.GetDynamicInfo(self, id, triggernum, data)
    ...
    if (self.watched[id] and self.watched[id][triggernum]) then
      for guid, durationInfo in pairs(self.watched[id][triggernum].players) do
        -- Need to check if cached data conforms to trigger
        if (durationInfo.expirationTime < bestExpirationTime and TestNonUniformSettings(
          durationInfo,
          data
        )) then
  ...
  function aura_cache.AssertAura(
  self,
    id,
    triggernum,
    guid,
      ...
      for triggernum, data in pairs(triggers) do
        if (not data.specificUnit or UnitIsUnit(data.unit, unit)) then
          ...
          if data.fullscan then
            ...
            aura_scan_cache[unit][filter] =
              aura_scan_cache[unit][filter] or { up_to_date = 0 }
            ...
            while name do
              ...
              if (aura_scan_cache[unit][filter].up_to_date < index) then
                ...
                tooltip, _, tooltipSize1, tooltipSize2, tooltipSize3 =
                  WeakAuras.GetAuraTooltipInfo(unit, index, filter)
                tooltipSize = { tooltipSize1, tooltipSize2, tooltipSize3 }
                aura_scan_cache[unit][filter][index] =
                  aura_scan_cache[unit][filter][index] or {}
              ...
              if (name and (not data.count or count and data.count(
                count
              )) and (data.ownOnly ~= false or not UnitIsUnit(
                "player",
                unitCaster or ""
              )) and data.scanFunc(
                name,
                ...
                if data.remFunc then
                  ...
                  if (remaining > data.rem) then
                    WeakAuras.ScheduleAuraScan(
                      unit,
                      time + (remaining - data.rem)
                    )
                  end
                end
                ...
                if checkPassed then
                  ...
                  if data.autoclone then
                    local cloneId =
                      name .. spellId .. "-" .. (casGUID or "unknown")
                    if WeakAuras.SetAuraVisibility(
                      id,
                      triggernum,
                      cloneId,
                      data.buffShowOn,
            ...
            for index, checkname in pairs(data.names) do
              ...
              if (name and (not data.count or data.count(
                count
              )) and (data.ownOnly ~= false or not UnitIsUnit(
                "player",
                unitCaster or ""
            ...
            if data.group_role then
              satisfies_role =
                data.group_role == "ANY" or UnitGroupRolesAssigned(
                  unit
                ) == data.group_role
            end
            local satisfies_ignoreSelf =
              not data.ignoreSelf or not UnitIsUnit(unit, "player")
            ...
            if active then
              ...
              if (aura_object and not data.specificUnit) then
                ...
                local nameFromTrigger, iconFromTrigger
                nameFromTrigger, iconFromTrigger =
                  GetNameAndIconFromTrigger(data)
                if WeakAuras.SetAuraVisibility(
                  id,
                  triggernum,
                  nil,
                  ...
                  if data.groupclone then
                    for guid, playerName in pairs(aura_cache.players) do
                      local duration,
                        ...
                        unitCaster
                      =
                        aura_object:GetPlayerDynamicInfo(
                          id,
                          triggernum,
                          guid,
                          data
                    ...
                    local duration,
                      ...
                      spellId,
                      unitCaster
                    = aura_object:GetDynamicInfo(id, triggernum, data)

                    if (data.name_info == "players") then
                      local affected =
                        aura_object:GetAffected(id, triggernum, data)
                      local num = 0
                      name = ""
                      for affected_name, _ in pairs(affected) do
                        local space = affected_name:find(" ")
                        name =
                          name .. (space and affected_name:sub(
                            1,
                            space - 1
                          ) .. "*" or affected_name) .. ", "
                        num = num + 1
                    ...
                    elseif (data.name_info == "nonplayers") then
                      local unaffected =
                        aura_object:GetUnaffected(id, triggernum, data)
                      local num = 0
                      name = ""
                      for unaffected_name, _ in pairs(unaffected) do
                        local space = unaffected_name:find(" ")
                        name =
                          name .. (space and unaffected_name:sub(
                            1,
                            space - 1
                          ) .. "*" or unaffected_name) .. ", "
                        num = num + 1
...
do
  ...
  local function updateRegion(id, data, triggernum, GUID)
    ...
    if (auradata and auradata.unitName) then
      ...
      local icon =
        auradata.icon or WeakAuras.GetDynamicIconCache(
          auradata.name
    ...
    for id, triggers in pairs(loaded_auras[spellName]) do
      ...
      for triggernum, data in pairs(triggers) do
        ...
        if (detected and (data.spellId == nil or data.spellId == spellId)) then
          ...
          updateTriggerState =
            updateRegion(id, data, triggernum, destGUID) or updateTriggerState
        end
      end
      if updateTriggerState then
        WeakAuras.UpdatedTriggerState(id)
      ...
      if (message == "SPELL_AURA_APPLIED" or message == "SPELL_AURA_REFRESH" or message == "SPELL_AURA_APPLIED_DOSE" or message == "SPELL_AURA_REMOVED_DOSE") then
        ...
        if unit then
          ...
          for id, triggers in pairs(loaded_auras[spellName]) do
            ...
            local updateTriggerState = false
            for triggernum, data in pairs(triggers) do
              if (not data.ownOnly or UnitIsUnit(
                sourceName or "",
                "player"
              )) then
                ...
                if (message == "SPELL_AURA_APPLIED_DOSE" or message == "SPELL_AURA_REMOVED_DOSE") then
                  data.GUIDs[destGUID].duration =
                    data.GUIDs[destGUID].duration or 0
                  data.GUIDs[destGUID].expirationTime =
                    data.GUIDs[destGUID].expirationTime or math.huge
                ...
                data.GUIDs[destGUID].spellId = spellId
                data.GUIDs[destGUID].unitCaster =
                  sourceName and UnitName(sourceName)
      ...
      for spellName, auras in pairs(loaded_auras) do
        if not WeakAuras.unit_types[spellName] then
          for id, triggers in pairs(auras) do
            ...
            for triggernum, data in pairs(triggers) do
              local filter =
                data.debuffType .. (data.ownOnly and "|PLAYER" or "")

              local detected
              local name, icon, count, duration, expirationTime, unitCaster, _
              for i = 1, BUFF_MAX_DISPLAY do
                name, icon, count, _, duration, expirationTime, unitCaster =
                  UnitAura(uid, i, filter)
                if not name then
                  break
                end
                if name == spellName then
              ...
              if detected then
                ...
                data.GUIDs[guid].unitCaster =
                  unitCaster and UnitName(unitCaster)
                updateTriggerState =
                  updateRegion(id, data, triggernum, guid) or updateTriggerState
              end
            end
            if updateTriggerState then
              WeakAuras.UpdatedTriggerState(id)
  ...
  function WeakAuras.ScheduleAuraScan(unit, fireTime)
    ...
    if not scheduled_scans[unit][fireTime] then
      ...
      local doScan = function()
        WeakAuras.debug(
          "Performing aura scan for " .. unit .. " at " .. fireTime .. " (" .. GetTime() .. ")"
        )
        scheduled_scans[unit][fireTime] = nil
        WeakAuras.ScanAuras(unit)
      end
      scheduled_scans[unit][fireTime] =
        timer:ScheduleTimerFixed(doScan, fireTime - GetTime() + 0.1)
    end
  end
end
...
frame:SetScript("OnEvent", function(frame, event, arg1, arg2, ...)
  ...
  elseif (event == "UNIT_AURA") then
    if (loaded_auras[arg1] or (loaded_auras["group"] and (arg1:sub(
      1,
      4
    ) == "raid" or arg1:sub(
      1,
      5
    ) == "party" or arg1 == "player")) or (loaded_auras["boss"] and arg1:sub(
      1,
      4
    ) == "boss") or (loaded_auras["arena"] and arg1:sub(1, 5) == "arena")) then
    ...
    if (type(trigger) == "table") then
      ...
      if (triggerType == "aura") then
        ...
        if trigger.useCount then
          countFuncStr =
            function_strings.count:format(
              trigger.countOperator or ">=",
              tonumber(trigger.count) or 0
            )
        ...
        if (trigger.unit == "group") then
          ...
          if (trigger.group_countOperator and count and countType) then
            if (countType == "whole") then
              ...
              group_countFuncStr =
                function_strings.count_fraction:format(
                  trigger.group_countOperator,
                  count
                )
            end
        ...
        if trigger.fullscan then
          scanFunc = function(name, tooltip, isStealable, spellId, debuffClass)
            if ((not trigger.use_name or (trigger.name and trigger.name ~= "" and (trigger.name_operator == "==" and name == trigger.name or trigger.name_operator == "find('%s')" and name:find(
              trigger.name
            ) or trigger.name_operator == "match('%s')" and name:match(
              trigger.name
            )))) and (not trigger.use_tooltip or (trigger.tooltip and trigger.tooltip ~= "" and (trigger.tooltip_operator == "==" and tooltip == trigger.tooltip or trigger.tooltip_operator == "find('%s')" and tooltip:find(
              trigger.tooltip
            ) or trigger.tooltip_operator == "match('%s')" and tooltip:match(
              trigger.tooltip
            )))) and (not trigger.use_stealable or isStealable) and (not trigger.use_spellId or spellId == tonumber(
              trigger.spellId
...
local function CanGroupShowWithZero(data, triggernum)
  ...
  if (trigger.unit == "group") then
    ...
    if (trigger.group_countOperator and count and countType) then
      if (countType == "whole") then
        group_countFuncStr =
          function_strings.count:format(trigger.group_countOperator, count)
      else
        group_countFuncStr =
          function_strings.count_fraction:format(
            trigger.group_countOperator,
...
function BuffTrigger.CanHaveClones(data, triggernum)
  local trigger = data.triggers[triggernum].trigger
  return (trigger.fullscan and trigger.autoclone) or (trigger.unit == "group" and trigger.groupclone) or (trigger.unit == "multi")
end
...
function BuffTrigger.SetToolTip(trigger, state)
  ...
  if (trigger.unit == "group" and trigger.name_info ~= "aura" and not trigger.groupclone) then
    ...
    if (trigger.name_info == "players") then
      playerList =
        WeakAuras.aura_cache:GetAffected(state.id, state.triggernum, data)
      name = L["Affected"] .. ":"
    elseif (trigger.name_info == "nonplayers") then
      playerList =
        WeakAuras.aura_cache:GetUnaffected(state.id, state.triggernum, data)
      name = L["Missing"] .. ":"
    else
      playerList = {}
    end
      ...
      if (numRaid > 0) then
        ...
        for subgroup, players in pairs(groupMembers) do
          ...
          for playerName, _ in pairs(players) do
            ...
            _, class =
              UnitClass((space and playerName:sub(1, space - 1) or playerName))
            classColor = WeakAuras.class_color_types[class]
            playersString =
              playersString .. (classColor or "") .. (space and playerName:sub(
                1,
                space - 1
              ) .. "*" or playerName) .. (classColor and "|r" or "") .. (next(
                players,
                playerName
              ) and ", " or "")
          end
  ...
  elseif (trigger.fullscan and trigger.unit ~= "group" and state.index) then
    local unit =
      trigger.unit == "member" and trigger.specificUnit or trigger.unit
...
function BuffTrigger.GetNameAndIcon(data, triggernum)
  ...
  if trigger.fullscan then
    ...
    elseif (not (trigger.buffShowOn == "showOnMissing" or CanGroupShowWithZero(
      data,
      triggernum
...
function BuffTrigger.GetAdditionalProperties(data, triggernum)
  ...
  ret =
    ret .. "|cFFFF0000%" .. triggernum .. ".unitCaster|r -" .. L["Caster"] .. "\n"
