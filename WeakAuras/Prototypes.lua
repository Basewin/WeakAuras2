...
local GetNumSpecializationsForClassID, GetSpecialization =
...
=
  ...
  UnitPower,
  UnitPowerMax
local UnitAlternatePowerInfo, UnitAlternatePowerTextureInfo =
  UnitAlternatePowerInfo,
  UnitAlternatePowerTextureInfo
local GetSpellInfo, GetItemInfo, GetItemCount, GetItemIcon =
  GetSpellInfo,
  GetItemInfo,
  GetItemCount,
  GetItemIcon
local GetShapeshiftFormInfo, GetShapeshiftForm =
  GetShapeshiftFormInfo,
  GetShapeshiftForm
local GetRuneCooldown, UnitCastingInfo, UnitChannelInfo =
  GetRuneCooldown,
  UnitCastingInfo,
  UnitChannelInfo
...
local function RangeCacheUpdate()
  ...
  RangeCacheStrings.friend, RangeCacheStrings.harm, RangeCacheStrings.misc =
    friendString,
    harmString,
    miscString
end
      ...
      if encounter_id then
        ...
        encounter_list =
          ("%s%s: %d\n"):format(
            encounter_list,
            boss,
            WeakAuras.encounter_table[ej_id]
          )
      end
      ej_index = ej_index + 1
...
function WeakAuras.CheckTalentByIndex(index)
  if WeakAuras.IsClassic() then
    ...
    local _, _, _, selected, _, _, _, _, _, _, known =
      GetTalentInfo(tier, column, 1)
...
function WeakAuras.CheckPvpTalentByIndex(index)
  if (index <= 3) then
    ...
    return select(3, GetPvpTalentInfoByID(pvpTalentId[index])) == select(
      3,
      GetPvpTalentInfoByID(talentSlotInfo.selectedTalentID)
    )
  else
...
function WeakAuras.CheckNumericIds(loadids, currentId)
  ...
  while startI do
    ...
    if (startI == 1 or tonumber(
      string.sub(loadids, startI - 1, startI - 1)
    ) == nil) then
      -- Either right at start, or character before is not a number
      if (endI == string.len(loadids) or tonumber(
        string.sub(loadids, endI + 1, endI + 1)
      ) == nil) then
        return true
      end
    end
...
function WeakAuras.ValidateNumeric(info, val)
  if val ~= nil and val ~= "" and (not tonumber(val) or tonumber(
    val
  ) >= 2 ^ 31) then
    return false
  end
...
function WeakAuras.IsSpellKnownIncludingPet(spell)
  ...
  if (baseSpell ~= spell) then
    return WeakAuras.IsSpellKnown(baseSpell) or WeakAuras.IsSpellKnown(
      baseSpell,
      true
    )
  end
...
local function valuesForTalentFunction(trigger)
  return function()
    ...
    if (single_class and WeakAuras.talent_types_specific[single_class] and single_spec and WeakAuras.talent_types_specific[single_class][single_spec]) then
      return WeakAuras.talent_types_specific[single_class][single_spec]
    elseif (WeakAuras.IsClassic() and single_class and WeakAuras.talent_types_specific[single_class] and WeakAuras.talent_types_specific[single_class]) then
  ...
  args = { {
        ...
        if (trigger.use_class == false and trigger.class and trigger.class.multi) then
          ...
          for class in pairs(trigger.class.multi) do
            ...
            min_specs =
              min(
                min_specs,
                GetNumSpecializationsForClassID(WeakAuras.class_ids[class])
              )
            num_classes = num_classes + 1
          end
          if (num_classes ~= 1) then
    ...
    values = function(trigger)
      return function()
        ...
        -- If a single specific class was found, load the specific list for it
        if (single_class and WeakAuras.pvp_talent_types_specific[single_class] and single_spec and WeakAuras.pvp_talent_types_specific[single_class][single_spec]) then
...
WeakAuras.event_prototypes = {
  ...
  Health = {
    ...
    args = { {
      ...
      required = true,
      enable = function(trigger)
        return WeakAuras.IsClassic() and trigger.use_showAbsorb
      end,
      hidden = WeakAuras.IsClassic()
    }, {
  ...
  Power = {
    ...
    init = function(trigger)
      ...
      ret =
        ret:format(
          trigger.unit,
          trigger.unit,
          trigger.use_powertype and trigger.powertype or "nil"
        )
      ...
      local canEnableShowCost =
        (not trigger.use_powertype or trigger.powertype ~= 99) and trigger.unit == "player"
    ...
    args = { {
      ...
      type = "select",
      values = function()
        return WeakAuras.IsClassic() and WeakAuras.power_types or WeakAuras.power_types_with_stagger
      end,
      init = "unitPowerType",
      test = "true",
      store = true,
    ...
    durationFunc = function(trigger)
      ...
      if powerType == 99 then
        ...
        if trigger.use_scaleStagger and trigger.scaleStagger then
          return UnitStagger(trigger.unit), math.max(
            1,
            UnitHealthMax(trigger.unit) * trigger.scaleStagger
          ), "fastUpdate"
        else
          return UnitStagger(trigger.unit), math.max(
            1,
            UnitHealthMax(trigger.unit)
          ), "fastUpdate"
        end -- combo points
      elseif (WeakAuras.IsClassic() and powerType == 4) then
        return GetComboPoints(trigger.unit, "target"), UnitPowerMax(
          trigger.unit,
          14
        ), true
      end
      ...
      local total =
        math.max(1, UnitPowerMax(trigger.unit, powerType, useThirdArg)) / pdm
  ...
  ["Alternate Power"] = {
    ...
    durationFunc = function(trigger)
      return UnitPower(trigger.unit, 10), math.max(
        1,
        UnitPowerMax(trigger.unit, 10)
      ), "fastUpdate"
    end,
    nameFunc = function(trigger)
      local _, _, _, _, _, _, _, _, _, _, name =
        UnitAlternatePowerInfo(trigger.unit)
      return name
    end,
  ...
  ["Combat Log"] = {
    ...
    canHaveAuto = true,
    statesParameter = "all",
    args = { {}, {}, {}, { -- timestamp ignored with _ argument -- messageType ignored with _ argument (it is checked before the dynamic function) -- hideCaster ignored with _ argument
      name = "sourceGUID",
      init = "arg",
      hidden = "true",
      test = "true",
      ...
      conditionTest = function(state, needle)
        return state and state.show and WeakAuras.CheckCombatLogFlags(
          state.sourceFlags,
          needle
        )
      end
      ...
      conditionType = "select",
      conditionTest = function(state, needle)
        return state and state.show and WeakAuras.CheckCombatLogFlagsReaction(
          state.sourceFlags,
          needle
        )
      end
      ...
      conditionType = "select",
      conditionTest = function(state, needle)
        return state and state.show and WeakAuras.CheckCombatLogFlagsObjectType(
          state.sourceFlags,
          needle
        )
      end
      ...
      conditionType = "select",
      conditionTest = function(state, needle)
        return state and state.show and WeakAuras.CheckRaidFlags(
          state.sourceRaidFlags,
          needle
        )
      end
    }, {
    ...
    enable = function(trigger)
      ...
      init = "arg",
      enable = function(trigger)
        return not WeakAuras.IsClassic() and trigger.subeventPrefix and (trigger.subeventPrefix:find(
          "SPELL"
        ) or trigger.subeventPrefix == "RANGE" or trigger.subeventPrefix:find(
          "DAMAGE"
        ))
      end,
      hidden = WeakAuras.IsClassic(),
      ...
      return WeakAuras.IsClassic() and trigger.subeventPrefix and (trigger.subeventPrefix:find(
        "SPELL"
      ) or trigger.subeventPrefix == "RANGE" or trigger.subeventPrefix:find(
        "DAMAGE"
      ))
    end }, { -- spellId ignored on classic
      name = "spellName",
      display = L["Spell Name"],
      type = "string",
      init = "arg",
      enable = function(trigger)
        return trigger.subeventPrefix and (trigger.subeventPrefix:find(
          "SPELL"
        ) or trigger.subeventPrefix == "RANGE" or trigger.subeventPrefix:find(
          "DAMAGE"
        ))
    ...
    end }, { -- spellSchool ignored with _ argument
    ...
    end }, { enable = function(trigger) -- source of absorb GUID ignored with SPELL_ABSORBED
      return trigger.subeventSuffix and (trigger.subeventSuffix == "_ABSORBED")
    end }, { enable = function(trigger) -- source of absorb Name ignored with SPELL_ABSORBED
      return trigger.subeventSuffix and (trigger.subeventSuffix == "_ABSORBED")
    end }, { enable = function(trigger) -- source of absorb Flags ignored with SPELL_ABSORBED
      return trigger.subeventSuffix and (trigger.subeventSuffix == "_ABSORBED")
    end }, { enable = function(trigger) -- source of absorb Raid Flags ignored with SPELL_ABSORBED
    ...
    end }, { -- extraSpellId ignored with SPELL_ABSORBED
      name = "extraSpellName",
      ...
      return trigger.subeventSuffix and (trigger.subeventSuffix == "_ABSORBED" or trigger.subeventSuffix == "_INTERRUPT" or trigger.subeventSuffix == "_DISPEL" or trigger.subeventSuffix == "_DISPEL_FAILED" or trigger.subeventSuffix == "_STOLEN" or trigger.subeventSuffix == "_AURA_BROKEN_SPELL")
    end }, { -- extraSchool ignored with _ argument
      name = "auraType",
      display = L["Aura Type"],
      type = "select",
      init = "arg",
      ...
      store = true,
      enable = function(trigger)
        return trigger.subeventSuffix and (trigger.subeventSuffix:find(
          "AURA"
        ) or trigger.subeventSuffix == "_DISPEL" or trigger.subeventSuffix == "_STOLEN")
      end,
      conditionType = "select"
    }, {
      name = "amount",
      ...
      init = "arg",
      enable = function(trigger)
        return trigger.subeventSuffix and trigger.subeventPrefix and (trigger.subeventSuffix == "_ABSORBED" or trigger.subeventSuffix == "_DAMAGE" or trigger.subeventSuffix == "_MISSED" or trigger.subeventSuffix == "_HEAL" or trigger.subeventSuffix == "_ENERGIZE" or trigger.subeventSuffix == "_DRAIN" or trigger.subeventSuffix == "_LEECH" or trigger.subeventPrefix:find(
          "DAMAGE"
        ))
      end,
      store = true,
      ...
      return trigger.subeventSuffix and trigger.subeventPrefix and (trigger.subeventSuffix == "_DAMAGE" or trigger.subeventPrefix == "DAMAGE_SHIELD" or trigger.subeventPrefix == "DAMAGE_SPLIT")
    end }, { -- damage school ignored with _ argument
      name = "resisted",
      display = L["Resisted"],
      type = "number",
      init = "arg",
      ...
      init = "arg",
      enable = function(trigger)
        return trigger.subeventSuffix and (trigger.subeventSuffix == "_EXTRA_ATTACKS" or trigger.subeventSuffix:find(
          "DOSE"
        ))
      end,
      store = true,
    ...
    end }, { -- unknown argument for _ENERGIZE ignored
      name = "powerType",
      display = L["Power Type"],
      type = "select",
      init = "arg",
    ...
    end }, { -- failedType ignored with _ argument - theoretically this is not necessary because it is the last argument in the event, but it is added here for completeness
      name = "cloneId",
  ...
  ["Spell Activation Overlay"] = {
    type = "status",
    events = {},
    internal_events = { "WA_UPDATE_OVERLAY_GLOW" },
  ...
  ["Cooldown Progress (Spell)"] = {
    ...
    loadFunc = function(trigger)
      ...
      if trigger.use_exact_spellName then
        ...
        spellName =
          type(trigger.spellName) == "number" and GetSpellInfo(
            trigger.spellName
          ) or trigger.spellName
      end
    ...
    args = { {}, { -- Ignore first argument (id)
      ...
      conditionType = "bool",
      conditionTest = function(state, needle)
        return state and state.show and (IsUsableSpell(
          state.spellname
        ) == (needle == 1))
      end,
      conditionEvents = {
      ...
      conditionType = "bool",
      conditionTest = function(state, needle)
        return state and state.show and (select(
          2,
      ...
      conditionType = "bool",
      conditionTest = function(state, needle)
        return state and state.show and (UnitExists(
          "target"
        ) and state.spellname and WeakAuras.IsSpellInRange(
          state.spellname,
          "target"
        ) == needle)
      end,
      conditionEvents = { "PLAYER_TARGET_CHANGED", "WA_SPELL_RANGECHECK" }
    }, {
      hidden = true,
      test = "genericShowOn"
    } },
  ...
  ["Cooldown Ready (Spell)"] = {
    type = "event",
    events = {},
    internal_events = { "SPELL_COOLDOWN_READY" },
  ...
  ["Charges Changed (Spell)"] = {
    type = "event",
    events = {},
    internal_events = { "SPELL_CHARGES_CHANGED" },
    ...
    args = { {
      ...
      conditionTest = function(state, needle)
        return state and state.show and WeakAuras.CheckChargesDirection(
          state.direction,
          needle
        )
      end
  ...
  ["Cooldown Progress (Item)"] = {
    ...
    loadFunc = function(trigger)
      ...
      local itemName =
        type(
          trigger.itemName
        ) == "number" and trigger.itemName or "[[" .. trigger.itemName .. "]]"
    ...
    args = { {
      ...
      conditionType = "bool",
      conditionTest = function(state, needle)
        return state and state.show and (UnitExists("target") and IsItemInRange(
          state.itemname,
          "target"
        )) == (needle == 1)
      end,
    ...
    durationFunc = function(trigger)
      local startTime, duration =
        WeakAuras.GetItemCooldown(
          type(trigger.itemName) == "number" and trigger.itemName or 0,
          trigger.use_showgcd
        )
  ...
  ["Cooldown Progress (Equipment Slot)"] = {
    ...
    stacksFunc = function(trigger)
      ...
      if ((count == 1) and not GetInventoryItemTexture(
        "player",
        trigger.itemSlot or 0
      )) then
        count = 0
  ...
  ["Cooldown Ready (Item)"] = {
    type = "event",
    events = {},
    internal_events = { "ITEM_COOLDOWN_READY" },
  ...
  ["Cooldown Ready (Equipment Slot)"] = {
    type = "event",
    events = {},
    internal_events = { "ITEM_SLOT_COOLDOWN_READY" },
  ...
  ["BigWigs Message"] = {
    type = "event",
    events = {},
    internal_events = { "BigWigs_Message" },
  ...
  ["Action Usable"] = {
    ...
    internal_events = { "SPELL_COOLDOWN_CHANGED" },
    ...
    init = function(trigger)
      ...
      if trigger.use_targetRequired then
        ret =
          ret .. "active = active and WeakAuras.IsSpellInRange(spellName or '', 'target')\n"
      end
    ...
    stacksFunc = function(trigger)
      local charges, maxCharges, spellCount =
        WeakAuras.GetSpellCharges(trigger.realSpellName)
  ...
  ["Talent Known"] = {
    ...
    events = function()
      local events
      if WeakAuras.IsClassic() then
        events = { "CHARACTER_POINTS_CHANGED", "SPELLS_CHANGED" }
      else
        events = { "PLAYER_TALENT_UPDATE" }
      end
  ...
  Totem = {
    ...
    internal_events = { "COOLDOWN_REMAINING_CHECK" },
    ...
    triggerFunction = function(trigger)
      ...
      local totemName =
        tonumber(trigger.totemName) and GetSpellInfo(
          tonumber(trigger.totemName)
        ) or trigger.totemName
      ret =
        ret:format(
  ...
  ["Item Count"] = {
    type = "status",
    events = {
      events = { "BAG_UPDATE", "BAG_UPDATE_COOLDOWN", "PLAYER_ENTERING_WORLD" }
    },
    internal_events = { "ITEM_COUNT_UPDATE" },
    ...
    durationFunc = function(trigger)
      local count =
        GetItemCount(
          trigger.itemName,
          trigger.use_includeBank,
          trigger.use_includeCharges
        )
      return count, 0, true
    end,
    stacksFunc = function(trigger)
  ...
  ["Stance/Form/Aura"] = {
    type = "status",
    events = {
      events = { "UPDATE_SHAPESHIFT_FORM", "UPDATE_SHAPESHIFT_COOLDOWN" }
    },
  ...
  ["Item Set"] = {
    ...
    durationFunc = function(trigger)
      return WeakAuras.GetNumSetItemsEquipped(
        trigger.itemSetId and tonumber(trigger.itemSetId) or 0
      )
    end,
    nameFunc = function(trigger)
      return select(
        3,
        WeakAuras.GetNumSetItemsEquipped(
          trigger.itemSetId and tonumber(trigger.itemSetId) or 0
        )
      )
    end
  ...
  ["Equipment Set"] = {
    ...
    init = function(trigger)
      ...
      local itemSetName =
        type(
          trigger.itemSetName
        ) == "string" and ("[=[" .. trigger.itemSetName .. "]=]") or "nil"
      ...
      return ret:format(
        trigger.use_itemSetName and "true" or "false",
        itemSetName,
        trigger.use_inverse and "true" or "false",
    ...
    nameFunc = function(trigger)
      return WeakAuras.GetEquipmentSetInfo(
        trigger.use_itemSetName and trigger.itemSetName or nil,
        trigger.use_partial
      )
    end,
    iconFunc = function(trigger)
      local _, icon =
        WeakAuras.GetEquipmentSetInfo(
          trigger.use_itemSetName and trigger.itemSetName or nil,
          trigger.use_partial
        )
      return icon
    ...
    durationFunc = function(trigger)
      local _, _, numEquipped, numItems =
        WeakAuras.GetEquipmentSetInfo(
          trigger.use_itemSetName and trigger.itemSetName or nil,
          trigger.use_partial
        )
  ...
  ["Threat Situation"] = {
    ...
    init = function(trigger)
      ...
      return ret:format(
        trigger.threatUnit and trigger.threatUnit ~= "none" and "[[" .. trigger.threatUnit .. "]]" or "nil"
      )
    end,
    args = { {
      name = "threatUnit",
  ...
  Cast = {
    ...
    events = function(trigger)
      ...
      if WeakAuras.IsClassic() and trigger.unit ~= "player" then
        LibClassicCasterino.RegisterCallback(
          "WeakAuras",
          "UNIT_SPELLCAST_START",
          WeakAuras.ScanUnitEvents
        )
        LibClassicCasterino.RegisterCallback(
          "WeakAuras",
    ...
    internal_events = function(trigger)
      local result =
        { "CAST_REMAINING_CHECK", "WA_DELAYED_PLAYER_ENTERING_WORLD" }
      if WeakAuras.IsClassic() and trigger.unit ~= "player" then
    ...
    args = { {
      ...
      conditionTest = function(state, unit, op)
        return state and state.show and (UnitIsUnit(
          state.sourceUnit,
          unit
        ) == (op == "=="))
      end,
      ...
      conditionType = "unit",
      conditionTest = function(state, unit, op)
        return state and state.show and state.destUnit and (UnitIsUnit(
          state.destUnit,
          unit
        ) == (op == "=="))
      end,
      ...
      init = "false",
      enable = function(trigger)
        return not trigger.use_inverse and (trigger.unit == "nameplate" or trigger.unit == "arena" or trigger.unit == "boss" or trigger.unit == "group")
      end
    }, {
      name = "inverse",
      display = L["Inverse"],
  ...
  ["Queued Action"] = {
    ...
    internal_events = { "ACTIONBAR_SLOT_CHANGED", "ACTIONBAR_PAGE_CHANGED" },
  ...
  ["Range Check"] = {
    ...
    init = function(trigger)
      ...
      if trigger.use_range then
        ...
        if (trigger.range_operator == "<=") then
          ret =
            ret .. "triggerResult = max <= " .. tostring(trigger.range) .. "\n"
        else
          ret =
            ret .. "triggerResult = min >= " .. tostring(trigger.range) .. "\n"
        end
      end
    ...
    args = { {
      ...
      conditionTest = function(state, needle, needle2)
        return state and state.show and WeakAuras.CheckRange(
          state.unit,
          needle,
          needle2
        )
...
WeakAuras.dynamic_texts = {
  p = { func = function(state, region)
    ...
    if state.progressType == "timed" then
      ...
      elseif remaining > 0 then
        local progressPrecision =
          region.progressPrecision and math.abs(region.progressPrecision) or 1
  ...
  t = { func = function(state, region)
    ...
    if state.progressType == "timed" then
      ...
      elseif duration > 0 then
        local totalPrecision =
          region.totalPrecision and math.abs(region.totalPrecision) or 1
  ...
  i = { func = function(state)
    if not state or not state.icon then
      return "|TInterface\\Icons\\INV_Misc_QuestionMark:12:12:0:0:64:64:4:60:4:60|t"
    end
    return "|T" .. state.icon .. ":12:12:0:0:64:64:4:60:4:60|t"
  end },
  s = { func = function(state)
