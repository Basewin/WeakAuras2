...
-- WoW APIs
local GetTalentInfo, IsAddOnLoaded, InCombatLockdown =
  GetTalentInfo,
  IsAddOnLoaded,
  InCombatLockdown
...
=
  ...
  GetFrameCPUUsage,
  debugprofilestop
local debugstack, IsSpellKnown, GetFileIDFromPath =
  debugstack,
  IsSpellKnown,
  GetFileIDFromPath
  ...
  if (delay < WeakAuras.maxTimerDuration) then
    if delay + GetTime() > WeakAuras.maxUpTime then
      WeakAuras.prettyPrint(
        WeakAuras.L["Can't schedule timer with %i, due to a World of Warcraft Bug with high computer uptime. (Uptime: %i). Please restart your Computer."]:format(
          delay,
          GetTime()
        )
      )
...
function WeakAuras.LoadOptions(msg)
  if not IsAddOnLoaded("WeakAurasOptions") then
    ...
    elseif InCombatLockdown() then
      ...
      queueshowooc = msg or ""
      WeakAuras.frames["Addon Initialization Handler"]:RegisterEvent(
      ...
      if not loaded then
        ...
        print(
          WeakAuras.printPrefix .. "Options could not be loaded, the addon is " .. reason
        )
        return false
      end
    end
...
function WeakAuras.PrintHelp()
  ...
  print(
    L["If you require additional assistance, please open a ticket on GitHub or visit our Discord at https://discord.gg/wa2!"]
  )
end
...
function WeakAuras.debug(msg, level)
  if db.debug then
    ...
    msg =
      (type(
        msg
      ) == "string" and msg) or (msg and "Invalid debug message of type " .. type(
        msg
      )) or "Debug message not specified"
...
name,
  ...
  if not name then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - name is not defined",
      2
    )
  elseif (type(name) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - name is not a string",
      2
    )
  elseif not createFunction then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - creation function is not defined",
      2
    )
  elseif (type(createFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - creation function is not a function",
      2
    )
  elseif not modifyFunction then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - modification function is not defined",
      2
    )
  elseif (type(modifyFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - modification function is not a function",
      2
    )
  elseif not default then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - default options are not defined",
      2
    )
  elseif (type(default) ~= "table") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - default options are not a table",
      2
    )
  elseif (type(default) ~= "table" and type(default) ~= "nil") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionType - properties options are not a table",
      2
    )
  elseif regionTypes[name] then
    error(
      'Improper arguments to WeakAuras.RegisterRegionType - region type "' .. name .. '" already defined',
      2
    )
  else
    regionTypes[name] = {
...
name,
  ...
  if not name then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - name is not defined",
      2
    )
  elseif (type(name) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - name is not a string",
      2
    )
  elseif not displayName then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - display name is not defined" .. " " .. name,
      2
    )
  elseif (type(displayName) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - display name is not a string",
      2
    )
  elseif not supportFunction then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - support function is not defined",
      2
    )
  elseif (type(supportFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - support function is not a function",
      2
    )
  elseif not createFunction then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - creation function is not defined",
      2
    )
  elseif (type(createFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - creation function is not a function",
      2
    )
  elseif not modifyFunction then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - modification function is not defined",
      2
    )
  elseif (type(modifyFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - modification function is not a function",
      2
    )
  elseif not onAcquire then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - onAcquire function is not defined",
      2
    )
  elseif (type(onAcquire) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - onAcquire function is not a function",
      2
    )
  elseif not onRelease then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - onRelease function is not defined",
      2
    )
  elseif (type(onRelease) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - onRelease function is not a function",
      2
    )
  elseif not default then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - default options are not defined",
      2
    )
  elseif (type(default) ~= "table" and type(default) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionType - default options are not a table or a function",
      2
    )
  elseif (addDefaultsForNewAura and type(
    addDefaultsForNewAura
  ) ~= "function") then
    error(
  ...
  elseif subRegionTypes[name] then
    error(
      'Improper arguments to WeakAuras.RegisterSubRegionType - region type "' .. name .. '" already defined',
      2
    )
  else
...
name,
  ...
  if not name then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - name is not defined",
      2
    )
    -- We only want to create one icon and reparent it as needed
  elseif (type(name) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - name is not a string",
      2
    )
  elseif not createFunction then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - creation function is not defined",
      2
    )
  elseif (type(createFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - creation function is not a function",
      2
    )
  elseif not icon then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - icon is not defined",
      2
    )
  elseif not (type(icon) == "string" or type(icon) == "function") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - icon is not a string or a function",
      2
    )
  elseif not displayName then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - display name is not defined" .. " " .. name,
      2
    )
  elseif (type(displayName) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - display name is not a string",
      2
    )
  elseif (getAnchors and type(getAnchors) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterRegionOptions - anchors is not a function",
      2
    )
  elseif regionOptions[name] then
    error(
      'Improper arguments to WeakAuras.RegisterRegionOptions - region type "' .. name .. '" already defined',
      2
    )
  else
...
function WeakAuras.RegisterSubRegionOptions(name, createFunction, description)
  if not name then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionOptions - name is not defined",
      2
    )
  elseif (type(name) ~= "string") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionOptions - name is not a string",
      2
    )
  elseif not createFunction then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionOptions - creation function is not defined",
      2
    )
  elseif (type(createFunction) ~= "function") then
    error(
      "Improper arguments to WeakAuras.RegisterSubRegionOptions - creation function is not a function",
      2
    )
  elseif subRegionOptions[name] then
    error(
      'Improper arguments to WeakAuras.RegisterSubRegionOptions - region type "' .. name .. '" already defined',
      2
    )
  else
...
function WeakAuras.ConstructFunction(prototype, trigger, skipOptional)
  ...
  for index, arg in pairs(prototype.args) do
    ...
    if enable then
      ...
      if not (arg.name or arg.hidden) then
        tinsert(input, "_")
        -- Do nothing -- multi selection -- single selection
      else
        ...
        elseif (arg.hidden or arg.type == "tristate" or arg.type == "toggle" or arg.type == "tristatestring" or (arg.type == "multiselect" and trigger["use_" .. name] ~= nil) or ((trigger["use_" .. name] or arg.required) and trigger[name])) then
          ...
          elseif (arg.type == "multiselect") then
              ...
              if (trigger[name] and trigger[name].multi) then
                ...
                for value, _ in pairs(trigger[name].multi) do
                  if not arg.test then
                    ...
                    test =
                      test .. arg.test:format(
                        tonumber(value) or "[[" .. value .. "]]"
                      ) .. " or "
                  end
                  any = true
                end
            ...
            elseif trigger["use_" .. name] then
              ...
              if not arg.test then
                test =
                  trigger[name] and trigger[name].single and "(" .. name .. "==" .. (tonumber(
                    value
                  ) or "[[" .. value .. "]]") .. ")"
              else
                test =
                  trigger[name] and trigger[name].single and "(" .. arg.test:format(
                    tonumber(value) or "[[" .. value .. "]]"
                  ) .. ")"
              end
            end
          ...
          elseif (arg.type == "spell") then
            if arg.showExactOption then
              test =
                "(" .. arg.test:format(
                  trigger[name],
                  tostring(trigger["use_exact_" .. name]) or "false"
                ) .. ")"
            else
              test = "(" .. arg.test:format(trigger[name]) .. ")"
            end
          ...
          elseif (arg.type == "longstring" and trigger[name .. "_operator"]) then
            if (trigger[name .. "_operator"] == "==") then
              ...
              test =
                "(" .. name .. ":" .. trigger[name .. "_operator"]:format(
                  trigger[name]
                ) .. ")"
            end
          else
  ...
  ret = ret .. "if("
  ret =
    ret .. ((#required > 0) and table.concat(
      required,
      " and "
    ) .. " and " or "")
...
function WeakAuras.GetActiveConditions(id, cloneId)
  triggerState[id].activatedConditions[cloneId] =
    triggerState[id].activatedConditions[cloneId] or {}
...
local function formatValueForAssignment(vtype, value, pathToCustomFunction)
  ...
  elseif (vtype == "chat") then
    if (value and type(value) == "table") then
      return string.format(
        "{message_type = %q, message = %q, message_dest = %q, message_channel = %q, message_custom = %s}",
        tostring(value.message_type),
        tostring(value.message or ""),
        tostring(value.message_dest),
        tostring(value.message_channel),
  ...
  elseif (vtype == "sound") then
    if (value and type(value) == "table") then
      return string.format(
        "{ sound = %q, sound_channel = %q, sound_path = %q, sound_kit_id = %q, sound_type = %q, %s}",
        tostring(value.sound or ""),
        tostring(value.sound_channel or ""),
        tostring(value.sound_path or ""),
        tostring(value.sound_kit_id or ""),
        tostring(value.sound_type or ""),
        value.sound_repeat and "sound_repeat = " .. tostring(
          value.sound_repeat
        ) or "nil"
      )
    end
...
function WeakAuras.scheduleConditionCheck(time, id, cloneId)
  conditionChecksTimers.recheckTime[id] =
    conditionChecksTimers.recheckTime[id] or {}
  conditionChecksTimers.recheckHandle[id] =
    conditionChecksTimers.recheckHandle[id] or {}
  ...
  if (conditionChecksTimers.recheckHandle[id][cloneId] == nil) then
    conditionChecksTimers.recheckHandle[id][cloneId] = timer:ScheduleTimerFixed(
      function()
        conditionChecksTimers.recheckHandle[id][cloneId] = nil
        local region
...
local function CreateTestForCondition(input, allConditionsTemplate, usedStates)
  ...
  if (variable == "AND" or variable == "OR") then
    ...
    if input.checks then
      for i, subcheck in ipairs(input.checks) do
        local subtest, subrecheckCode =
          CreateTestForCondition(subcheck, allConditionsTemplate, usedStates)
        if subtest then
  ...
  if (trigger and variable and value) then
    ...
    local conditionTemplate =
      allConditionsTemplate[trigger] and allConditionsTemplate[trigger][variable]
    local ctype = conditionTemplate and conditionTemplate.type
    local test = conditionTemplate and conditionTemplate.test

    local stateCheck =
      "state[" .. trigger .. "] and state[" .. trigger .. "].show and "
    local stateVariableCheck =
      "state[" .. trigger .. "]." .. variable .. "~= nil and "
    if test then
      if value then
        tinsert(WeakAuras.customConditionTestFunctions, test)
        local testFunctionNumber = #WeakAuras.customConditionTestFunctions
        local valueString =
          type(value) == "string" and "[[" .. value .. "]]" or value
        local opString = type(op) == "string" and "[[" .. op .. "]]" or op
        check =
          "state and WeakAuras.customConditionTestFunctions[" .. testFunctionNumber .. "](state[" .. trigger .. "], " .. valueString .. ", " .. (opString or "nil") .. ")"
      end
    elseif (ctype == "number" and op) then
      local v = tonumber(value)
      if v then
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. op .. v
      end
    elseif (ctype == "timer" and op) then
      if (op == "==") then
        check =
          stateCheck .. stateVariableCheck .. "abs(state[" .. trigger .. "]." .. variable .. "- now -" .. value .. ") < 0.05"
      else
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. "- now" .. op .. value
      end
    elseif (ctype == "select" and op) then
      if tonumber(value) then
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. op .. tonumber(
            value
          )
      else
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. op .. "'" .. value .. "'"
      end
    elseif (ctype == "bool") then
      local rightSide = value == 0 and "false" or "true"
      check =
        stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. "==" .. rightSide
    elseif (ctype == "string") then
      if (op == "==") then
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. " == [[" .. value .. "]]"
      elseif (op == "find('%s')") then
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. ":find([[" .. value .. "]], 1, true)"
      elseif (op == "match('%s')") then
        check =
          stateCheck .. stateVariableCheck .. "state[" .. trigger .. "]." .. variable .. ":match([[" .. value .. "]], 1, true)"
      end
    end

    if (ctype == "timer" and value) then
      recheckCode =
        "  nextTime = state[" .. trigger .. "] and state[" .. trigger .. "]." .. variable .. " and (state[" .. trigger .. "]." .. variable .. " -" .. value .. ")\n"
      recheckCode =
        recheckCode .. "  if (nextTime and (not recheckTime or nextTime < recheckTime) and nextTime >= now) then\n"
...
local function CreateCheckCondition(
ret,
  condition,
  conditionNumber,
  allConditionsTemplate,
  debug
)
  local usedStates = {}
  local check, recheckCode =
    CreateTestForCondition(condition.check, allConditionsTemplate, usedStates)
  if check then
    ret = ret .. "    state = region.states\n"
    ret = ret .. "    if (" .. check .. ") then\n"
    ret =
      ret .. "      newActiveConditions[" .. conditionNumber .. "] = true;\n"
    ret = ret .. "    end\n"
  end
  if recheckCode then
    ret = ret .. recheckCode
...
local function GetBaseProperty(data, property, start)
  ...
  if next then
    return GetBaseProperty(
      data[string.sub(property, start, next - 1)],
      property,
      next + 1
    )
  end
...
local function CreateDeactivateCondition(
ret,
  condition,
  conditionNumber,
  data,
  ...
  if condition.changes then
    ret =
      ret .. "  if (activatedConditions[" .. conditionNumber .. "] and not newActiveConditions[" .. conditionNumber .. "]) then\n"
    if debug then
      ret =
        ret .. "    print('Deactivating condition " .. conditionNumber .. "' )\n"
    end
    for changeNum, change in ipairs(condition.changes) do
      if change.property then
        ...
        if (propertyData and propertyData.type and propertyData.setter) then
          ...
          ret =
            ret .. "    propertyChanges['" .. change.property .. "'] = " .. formatValueForAssignment(
              propertyData.type,
              GetBaseProperty(data, change.property)
            ) .. "\n"
          if debug then
            ret =
              ret .. "    print('- " .. change.property .. " " .. formatValueForAssignment(
                propertyData.type,
                GetBaseProperty(data, change.property)
              ) .. "')\n"
          end
...
local function CreateActivateCondition(
ret,
  id,
  condition,
  conditionNumber,
  ...
  if condition.changes then
    ...
    ret =
      ret .. "    if (not activatedConditions[" .. conditionNumber .. "]) then\n"
    if debug then
      ret =
        ret .. "      print('Activating condition " .. conditionNumber .. "' )\n"
    end
    -- non active => active
    for changeNum, change in ipairs(condition.changes) do
      if change.property then
        ...
        if (propertyData and propertyData.type) then
          ...
          elseif propertyData.action then
            ...
            if (WeakAuras.customConditionsFunctions[id] and WeakAuras.customConditionsFunctions[id][conditionNumber] and WeakAuras.customConditionsFunctions[id][conditionNumber].changes and WeakAuras.customConditionsFunctions[id][conditionNumber].changes[changeNum]) then
              pathToCustomFunction =
                string.format(
                  "WeakAuras.customConditionsFunctions[%q][%s].changes[%s]",
                  id,
                  conditionNumber,
                  changeNum
                )
            ...
            ret =
              ret .. "     region:" .. propertyData.action .. "(" .. formatValueForAssignment(
                propertyData.type,
                change.value,
                pathToCustomFunction
              ) .. ")" .. "\n"
            if debug then
              ret =
                ret .. "     print('# " .. propertyData.action .. "(" .. formatValueForAssignment(
                  propertyData.type,
                  change.value,
                  pathToCustomFunction
                ) .. "')\n"
    ...
    for changeNum, change in ipairs(condition.changes) do
      if change.property then
        ...
        if (propertyData and propertyData.type and propertyData.setter) then
          ret =
            ret .. "      if(propertyChanges['" .. change.property .. "'] ~= nil) then\n"
          ret =
            ret .. "        propertyChanges['" .. change.property .. "'] = " .. formatValueForAssignment(
              propertyData.type,
              change.value
    ...
    ret = ret .. "\n"
    ret =
      ret .. "  activatedConditions[" .. conditionNumber .. "] = newActiveConditions[" .. conditionNumber .. "]\n"
  end
...
function WeakAuras.LoadCustomActionFunctions(data)
  ...
  if data.actions then
    if (data.actions.init and data.actions.init.do_custom and data.actions.init.custom) then
      local func =
        WeakAuras.LoadFunction(
          "return function() " .. data.actions.init.custom .. "\n end",
          id,
          "initialization"
        )
    ...
    if data.actions.start then
      if (data.actions.start.do_custom and data.actions.start.custom) then
        local func =
          WeakAuras.LoadFunction(
            "return function() " .. data.actions.start.custom .. "\n end",
            id,
            "show action"
          )
      ...
      if (data.actions.start.do_message and data.actions.start.message_custom) then
        local func =
          WeakAuras.LoadFunction(
            "return " .. data.actions.start.message_custom,
            id,
            "show message"
          )
    ...
    if data.actions.finish then
      if (data.actions.finish.do_custom and data.actions.finish.custom) then
        local func =
          WeakAuras.LoadFunction(
            "return function() " .. data.actions.finish.custom .. "\n end",
            id,
            "hide action"
          )
      ...
      if (data.actions.finish.do_message and data.actions.finish.message_custom) then
        local func =
          WeakAuras.LoadFunction(
            "return " .. data.actions.finish.message_custom,
            id,
            "hide message"
          )
...
function WeakAuras.GetProperties(data)
  local properties
  local propertiesFunction =
    WeakAuras.regionTypes[data.regionType] and WeakAuras.regionTypes[data.regionType].properties
  ...
  if data.subRegions then
    ...
    for index, subRegion in ipairs(data.subRegions) do
      ...
      local propertiesFunction =
        subRegionTypeData and subRegionTypeData.properties
      local subProperties
      if (type(propertiesFunction) == "function") then
      ...
      if subProperties then
        for key, property in pairs(subProperties) do
          ...
          property.display =
            {
              subIndex[key] .. ". " .. subRegionTypeData.displayName,
              property.display,
    ...
    for conditionNumber, condition in ipairs(data.conditions) do
      if condition.changes then
        for changeIndex, change in ipairs(condition.changes) do
          if ((change.property == "chat" or change.property == "customcode") and type(
            change.value
          ) == "table" and change.value.custom) then
            ...
            if customFunc then
              WeakAuras.customConditionsFunctions[id][conditionNumber] =
                WeakAuras.customConditionsFunctions[id][conditionNumber] or {}
              WeakAuras.customConditionsFunctions[id][conditionNumber].changes =
                WeakAuras.customConditionsFunctions[id][conditionNumber].changes or {}
              WeakAuras.customConditionsFunctions[id][conditionNumber].changes[changeIndex]
              = customFunc
            end
          end
        end
...
function WeakAuras.ConstructConditionFunction(data)
  ...
  ret =
    ret .. "  local activatedConditions = WeakAuras.GetActiveConditions(id, cloneId)\n"
  ...
  ret =
    ret .. "    WeakAuras.scheduleConditionCheck(recheckTime, id, cloneId);\n"
  ret = ret .. "  end\n"
  ...
  for property, _ in pairs(usedProperties) do
    ...
    ret =
      ret .. "    " .. base .. properties[property].setter .. "(" .. arg1 .. formatValueForCall(
        properties[property].type,
        property
      ) .. ")\n"
    if debug then
      ret =
        ret .. "    print('Calling " .. properties[property].setter .. " with', " .. arg1 .. formatValueForCall(
          properties[property].type,
          property
        ) .. ")\n"
    end
...
local function runDynamicConditionFunctions(funcs)
  for id in pairs(funcs) do
    if (triggerState[id] and triggerState[id].show and checkConditions[id]) then
      local activeTriggerState =
        WeakAuras.GetTriggerStateForTrigger(id, triggerState[id].activeTrigger)
...
local function EvaluateCheckForRegisterForGlobalConditions(
id,
  check,
  allConditionsTemplate,
  register
  ...
  if (trigger == -2) then
    if check.checks then
      for _, subcheck in ipairs(check.checks) do
        EvaluateCheckForRegisterForGlobalConditions(
          id,
          subcheck,
          allConditionsTemplate,
          register
  ...
  elseif (trigger and variable) then
    ...
    if (conditionTemplate and conditionTemplate.events) then
      ...
      if (conditionTemplate.globalStateUpdate and not registeredGlobalFunctions[variable]) then
        ...
        for _, event in ipairs(conditionTemplate.events) do
          globalDynamicConditionFuncs[event] =
            globalDynamicConditionFuncs[event] or {}
          tinsert(
            globalDynamicConditionFuncs[event],
            conditionTemplate.globalStateUpdate
          )
        end
  ...
  for event in pairs(register) do
    if (event == "FRAME_UPDATE" or event == "WA_SPELL_RANGECHECK") then
      if not dynamicConditionsFrame.onUpdate then
        dynamicConditionsFrame:SetScript(
          "OnUpdate",
          handleDynamicConditionsOnUpdate
        )
...
function WeakAuras.CreateTalentCache()
  ...
  WeakAuras.talent_types_specific[player_class] =
    WeakAuras.talent_types_specific[player_class] or {}
  ...
  if WeakAuras.IsClassic() then
    for tab = 1, GetNumTalentTabs() do
      for num_talent = 1, GetNumTalents(tab) do
        ...
        if (talentName and talentIcon) then
          WeakAuras.talent_types_specific[player_class][talentId] =
            "|T" .. talentIcon .. ":0|t " .. talentName
        end
      end
    end
    ...
    WeakAuras.talent_types_specific[player_class][spec] =
      WeakAuras.talent_types_specific[player_class][spec] or {}
    ...
    for tier = 1, MAX_TALENT_TIERS do
      for column = 1, NUM_TALENT_COLUMNS do
        ...
        if (talentName and talentIcon) then
          WeakAuras.talent_types_specific[player_class][spec][talentId] =
            "|T" .. talentIcon .. ":0|t " .. talentName
        end
      end
    end
...
function WeakAuras.CreatePvPTalentCache()
  ...
  if (not player_class or not spec) then return end

  WeakAuras.pvp_talent_types_specific[player_class] =
    WeakAuras.pvp_talent_types_specific[player_class] or {}
  WeakAuras.pvp_talent_types_specific[player_class][spec] =
    WeakAuras.pvp_talent_types_specific[player_class][spec] or {}
  ...
  if slotInfo then
    WeakAuras.pvp_talent_types_specific[player_class][spec] =
      { formatTalent(3589), formatTalent(3588), formatTalent(3587), nil }

    local pvpSpecTalents = slotInfo.availableTalentIDs
    for i, talentId in ipairs(pvpSpecTalents) do
      WeakAuras.pvp_talent_types_specific[player_class][spec][i + 3] =
        formatTalent(talentId)
    end
  ...
  for id, aura in pairs(WeakAurasSaved.displays) do
    if not aura.ignoreWagoUpdate and aura.url and aura.url ~= "" then
      ...
      if slug and version then
        ...
        if wago and wago.wagoVersion and tonumber(
          wago.wagoVersion
        ) > (aura.skipWagoUpdate and tonumber(aura.skipWagoUpdate) or tonumber(
          version
        )) then
...
local function tooltip_draw()
  ...
  if WeakAurasCompanion then
    ...
    if count > 0 then
      tooltip:AddLine(" ")
      tooltip:AddLine(
        L["There are %i updates to your auras ready to be installed!"]:format(
          count
        )
      )
    end
  ...
  if not WeakAuras.IsOptionsOpen() then
    if paused then
      tooltip:AddLine(
        "|cFFFF0000" .. L["Paused"] .. " - " .. L["Shift-Click to resume addon execution."],
        0.2,
        1,
        0.2
      )
  ...
  tooltip:AddLine(
    L["|cffeda55fRight-Click|r to toggle performance profiling on or off."],
    0.2,
    1,
    0.2
  )
  tooltip:AddLine(
    L["|cffeda55fShift-Right-Click|r to show profiling results."],
    0.2,
    1,
    0.2
  )
...
local loginFinished, loginMessage =
  false,
...
function WeakAuras.Login(initialTime, takeNewSnapshots)
  ...
  -- check in case of a disconnect during an encounter.
    ...
    for id, data in pairs(db.displays) do
      if (id ~= data.id) then
        print(
          "|cFF8800FFWeakAuras|r detected a corrupt entry in WeakAuras saved displays - '" .. tostring(
            id
          ) .. "' vs '" .. tostring(data.id) .. "'"
        )
        data.id = id
  ...
  if initialTime then
    ...
    while coroutine.status(
      loginThread
    ) ~= "dead" and finishTime - startTime < 15000 do
...
loadedFrame:SetScript("OnEvent", function(self, event, addon)
  if (event == "ADDON_LOADED") then
    if (addon == ADDON_NAME) then
      ...
      if db.clearOldHistory ~= false and type(
        db.clearOldHistory
      ) ~= "number" then
    ...
    -- Schedule events that need to be handled some time after login -- No sounds while loading -- It seems that GetTalentInfo might give info about whatever class was previously being played, until PLAYER_ENTERING_WORLD
    ...
    elseif (event == "PLAYER_REGEN_ENABLED") then
      callback = function()
        if queueshowooc then
          ...
          queueshowooc = nil
          WeakAuras.frames["Addon Initialization Handler"]:UnregisterEvent(
            "PLAYER_REGEN_ENABLED"
          )
        end
      end
...
local function LoadEncounterInitScriptsImpl(id)
  if (WeakAuras.currentInstanceType ~= "raid") then return end
  if id then
    local data = db.displays[id]
    if (data and data.load.use_encounterid and not WeakAuras.IsEnvironmentInitialized(
      id
    ) and data.actions.init and data.actions.init.do_custom) then
    ...
    for id, data in pairs(db.displays) do
      if (data.load.use_encounterid and not WeakAuras.IsEnvironmentInitialized(
        id
      ) and data.actions.init and data.actions.init.do_custom) then
        WeakAuras.ActivateAuraEnvironment(id)
        WeakAuras.ActivateAuraEnvironment(nil)
...
function WeakAuras.LoadEncounterInitScripts(id)
  if not WeakAuras.IsLoginFinished() then
    if encounterScriptsDeferred[id] then return end
...
local function GetInstanceTypeAndSize()
  ...
  local _, instanceType, difficultyIndex, _, _, _, _, ZoneMapID =
    GetInstanceInfo()
  if inInstance then
    size = Type
...
local function scanForLoadsImpl(toCheck, event, arg1, ...)
  ...
  local encounter_id =
    WeakAuras.CurrentEncounter and WeakAuras.CurrentEncounter.id or 0
  ...
  if toCheck == nil or next(toCheck) == nil then return end
  ...
  local role =
    WeakAuras.IsClassic() and "none" or select(5, GetSpecializationInfo(spec))
  ...
  if not WeakAuras.IsClassic() then
    ...
    affixes =
      C_ChallengeMode.IsChallengeModeActive() and select(
        2,
...
function WeakAuras.ScanForLoads(toCheck, event, arg1, ...)
  if not WeakAuras.IsLoginFinished() then return end
...
function WeakAuras.UnloadAll()
  ...
  for id in pairs(loaded) do
    WeakAuras.CancelAnimation(
      WeakAuras.regions[id].region,
      true,
      true,
      true,
      true,
...
function WeakAuras.Rename(data, newid)
  ...
  conditionChecksTimers.recheckTime[newid] =
    conditionChecksTimers.recheckTime[oldid]
  conditionChecksTimers.recheckTime[oldid] = nil

  conditionChecksTimers.recheckHandle[newid] =
    conditionChecksTimers.recheckHandle[oldid]
  ...
  WeakAuras.customActionsFunctions[newid] =
    WeakAuras.customActionsFunctions[oldid]
  WeakAuras.customActionsFunctions[oldid] = nil

  WeakAuras.customConditionsFunctions[newid] =
    WeakAuras.customConditionsFunctions[oldid]
...
function WeakAuras.ResolveCollisions(onFinished)
  ...
  if (num > 0) then
    ...
    local function UpdateText(popup)
      popup.text:SetText(
        baseText .. (numResolved or "error") .. "/" .. (num or "error")
      )
    end
    ...
    StaticPopupDialogs["WEAKAURAS_RESOLVE_COLLISIONS"] = {
      ...
      OnAccept = function(self)
        ...
        WeakAuras.CollisionResolved(
          collisions[currentId][1],
          collisions[currentId][2],
          true
        )
    ...
    popup.icon:SetTexture(
      "Interface\\Addons\\WeakAuras\\Media\\Textures\\icon.blp"
    )
...
StaticPopupDialogs["WEAKAURAS_CONFIRM_REPAIR"] = {
  ...
  OnShow = function(self)
    if self.data.reason == "user" then
      self.text:SetText(
        L["Manual Repair Confirmation Dialog"]:format(WeakAuras.LastUpgrade())
      )
    else
      self.text:SetText(
        L["Automatic Repair Confirmation Dialog"]:format(
          WeakAuras.LastUpgrade()
        )
      )
    end
  end,
...
function WeakAuras.Modernize(data)
  ...
  if (data.internalVersion < 2) then
    ...
    if not data.numTriggers then
      data.numTriggers =
        1 + (data.additional_triggers and #data.additional_triggers or 0)
    end
    ...
    if load.class.single then
      load.class.single =
        class_agnosticize[load.class.single] or load.class.single
    end
    ...
    if (data.regionType == "aurabar") then
      data.displayTextLeft =
        data.displayTextLeft or (not data.auto and data.displayText) or "%n"
        ...
        if (data.orientation == "CLOCKWISE") then
          if data.inverse then
            data.startAngle, data.endAngle =
              ...
              360 - data.startAngle
            data.orientation =
              (data.orientation == "CLOCKWISE") and "ANTICLOCKWISE" or "CLOCKWISE"
          end
  ...
  if data.internalVersion < 7 then
    ...
    local activeTriggerMode =
      data.activeTriggerMode or WeakAuras.trigger_modes.first_active
  ...
  if data.internalVersion < 9 then
    local function repairCheck(check)
      if check and check.variable == "buffed" then
        local trigger =
          check.trigger and data.triggers[check.trigger] and data.triggers[check.trigger].trigger
        if trigger then
  ...
  if data.internalVersion < 14 then
    if data.triggers then
      for triggerId, triggerData in pairs(data.triggers) do
        if type(
          triggerData
        ) == "table" and triggerData.trigger and triggerData.trigger.debuffClass and type(
          triggerData.trigger.debuffClass
        ) == "string" and triggerData.trigger.debuffClass ~= "" then
  ...
  if data.internalVersion < 16 then
    ...
    if data.regionType == "progresstexture" then
      if type(data.foregroundTexture) == "string" then
        local textureId =
          GetFileIDFromPath(data.foregroundTexture:gsub("\\\\", "\\"))
        if textureId and textureId > 0 then
      ...
      if type(data.backgroundTexture) == "string" then
        local textureId =
          GetFileIDFromPath(data.backgroundTexture:gsub("\\\\", "\\"))
        if textureId and textureId > 0 then
  ...
  if data.internalVersion < 20 then
    if data.regionType == "aurabar" then
      ...
      local positions =
        orientationToPostion[data.orientation] or {
          "INNER_LEFT",
          "INNER_RIGHT"
        }
  ...
  for _, data in pairs(db.displays) do
    if type(data.uid) == "string" then
      if seenUIDs[data.uid] then
        if not silent then
          prettyPrint(
            'duplicate uid "' .. data.uid .. '" detected in saved variables between "' .. data.id .. '" and "' .. seenUIDs[data.uid].id .. '".'
          )
        end
    ...
    elseif data.uid ~= nil then
      if not silent then
        prettyPrint(
          'invalid uid detected in saved variables for "' .. data.id .. '"'
        )
      end
      data.uid = WeakAuras.GenerateUniqueID()
      seenUIDs[data.uid] = data
  ...
  for id, data in pairs(db.displays) do
    if data.parent then
      if data.controlledChildren then
        if not silent then
          prettyPrint(
            "detected corruption in saved variables: " .. id .. " is a group that thinks it's a parent."
          )
        end
        data.parent = nil
        parents[id] = data
      ...
      elseif not db.displays[data.parent] then
        if not silent then
          prettyPrint(
            "detected corruption in saved variables: " .. id .. " has a nonexistent parent."
          )
        end
        data.parent = nil
      elseif not db.displays[data.parent].controlledChildren then
        if not silent then
          prettyPrint(
            "detected corruption in saved variables: " .. id .. " thinks " .. data.parent .. " controls it, but " .. data.parent .. " is not a group."
          )
        end
        data.parent = nil
      else
    ...
    if data.parent then
      if idtable[data.parent] then
        if tContains(depends, data.parent) then
          error(
            "Circular dependency in WeakAuras.AddMany between " .. table.concat(
              depends,
              ", "
            )
          )
...
local function validateUserConfig(data, options, config)
  ...
  for index, option in ipairs(options) do
    if not customOptionIsValid(option) then
      prettyPrint(
        data.id .. " Custom Option #" .. index .. " in " .. data.id .. " has been detected as corrupt, and has been deleted."
      )
      corruptOptions[index] = true
      -- if k was not a number, then this was a simple group before
      -- if v is not a table, then this was likely a color option -- second iteration will fill table with defaults
      -- add missing entries
  ...
  for i = 1, #data.triggers do
    ...
    if trigger and trigger.type == "aura" then
      if type(trigger.spellName) == "number" then
        trigger.realSpellName =
          GetSpellInfo(trigger.spellName) or trigger.realSpellName
      end
      ...
      if trigger.spellIds then
        for i = 1, 10 do
          if trigger.spellIds[i] then
            ...
            trigger.names[i] =
              GetSpellInfo(trigger.spellIds[i]) or trigger.names[i]
          end
        end
      end
    end
...
local function pAdd(data, simpleChange)
  ...
  if simpleChange then
    ...
    if data.controlledChildren then
      ...
      local loadFuncStr, events =
        WeakAuras.ConstructFunction(load_prototype, data.load)
      for event, eventData in pairs(loadEvents) do
        eventData[id] = nil
      end
      ...
      local loadForOptionsFuncStr =
        WeakAuras.ConstructFunction(load_prototype, data.load, true)
      local loadFunc = WeakAuras.LoadFunction(loadFuncStr, id, "load")
      local loadForOptionsFunc =
        WeakAuras.LoadFunction(loadForOptionsFuncStr, id, "options load")
      local triggerLogicFunc
      if data.triggers.disjunctive == "custom" then
        triggerLogicFunc =
          WeakAuras.LoadFunction(
            "return " .. (data.triggers.customTriggerLogic or ""),
            id,
            "trigger combination"
          )
      end
      ...
      local checkCondtionsFunc =
        checkConditionsFuncStr and WeakAuras.LoadFunction(
          checkConditionsFuncStr,
          id,
          "condition checks"
        )
      debug(id .. " - Load", 1)
...
function WeakAuras.SetRegion(data, cloneId)
  ...
  if not regionType then
    ...
    if not regionTypes[regionType] then
      regionType = "fallback"
      print(
        'Improper arguments to WeakAuras.CreateRegion - regionType "' .. data.regionType .. '" is not supported'
      )
    end
    ...
    if not id then
      ...
      local anim_cancelled =
        loginFinished and WeakAuras.CancelAnimation(
          region,
          true,
          true,
          true,
      ...
      WeakAuras.regionPrototype.AddExpandFunction(
        data,
        region,
        cloneId,
        parent,
      ...
      if WeakAuras.CanHaveDuration(data) then
        data.animation.start.duration_type =
          data.animation.start.duration_type or "seconds"
        data.animation.main.duration_type =
          data.animation.main.duration_type or "seconds"
        data.animation.finish.duration_type =
          data.animation.finish.duration_type or "seconds"
      else
...
function WeakAuras.PerformActions(data, type, region)
  if (paused or WeakAuras.IsOptionsOpen()) then return end
  local actions
  if (type == "start") then
  ...
  if (actions.do_message and actions.message_type and actions.message and not squelch_actions) then
    local customFunc =
      WeakAuras.customActionsFunctions[data.id][type .. "_message"]
    WeakAuras.HandleChatAction(
...
local function noopErrorHandler()
end
...
function WeakAuras.UpdateAnimations()
  ...
  local errorHandler =
    WeakAuras.IsOptionsOpen() and noopErrorHandler or geterrorhandler()
    ...
    elseif (anim.duration_type == "relative") then
      ...
      if (not state or (state.progressType == "timed" and state.duration < 0.01) or (state.progressType == "static" and state.value < 0.01)) then
        ...
        elseif (state.progressType == "timed") then
          relativeProgress =
            1 - ((state.expirationTime - time) / state.duration)
        end
        relativeProgress =
  ...
  if valid then
    ...
    if animations[key] then
      ...
      selfPoint, anchor, anchorPoint, startX, startY =
        animations[key].selfPoint,
        animations[key].anchor,
        animations[key].anchorPoint,
        animations[key].startX,
        animations[key].startY
      ...
      startWidth, startHeight =
        animations[key].startWidth,
        animations[key].startHeight
      ...
      if (data and data.parent) then
        ...
        if (parentRegion and parentRegion.controlledRegions) then
          for index, regionData in pairs(parentRegion.controlledRegions) do
            ...
            if (childKey and childKey ~= tostring(
              region
            ) and animations[childKey] and animations[childKey].type == "main" and duration == animations[childKey].duration) then
...
function WeakAuras.CancelAnimation(
region,
  resetPos,
  resetAlpha,
  resetScale,
...
function WeakAuras.GetTriggerSystem(data, triggernum)
  local triggerType =
    data.triggers[triggernum] and data.triggers[triggernum].trigger.type
...
WeakAuras.CanHaveDuration =
  wrapTriggerSystemFunction("CanHaveDuration", "firstValue")
...
WeakAuras.GetNameAndIcon =
  wrapTriggerSystemFunction("GetNameAndIcon", "nameAndIcon")
WeakAuras.GetTriggerDescription =
  wrapTriggerSystemFunction("GetTriggerDescription", "call")

local wrappedGetOverlayInfo =
  wrapTriggerSystemFunction("GetOverlayInfo", "table")
  ...
  function WeakAuras.GetHiddenTooltip()
    if not hiddenTooltip then
      ...
      hiddenTooltip:AddFontStrings(
        hiddenTooltip:CreateFontString(
          "$parentTextLeft1",
          nil,
          "GameTooltipText"
        ),
...
function WeakAuras.RegisterTutorial(
name,
  displayName,
  description,
  icon,
...
function WeakAuras.SetFrameLevel(id, frameLevel)
  if (WeakAuras.frameLevels[id] == frameLevel) then return end
  if (WeakAuras.regions[id] and WeakAuras.regions[id].region) then
...
do
  ...
  function WeakAuras.SetFakeStates()
    if UpdateFakeTimesHandle then return end
  ...
  elseif (triggerState[id].disjunctive == "all" and triggerState[id].triggerCount == triggerState[id].numTriggers) then
    ...
    if (triggerState[id].disjunctive == "custom" and triggerState[id].triggerLogicFunc) then
      local ok, returnValue =
        xpcall(
          triggerState[id].triggerLogicFunc,
          geterrorhandler(),
          triggerState[id].triggers
        )
...
local function ApplyStatesToRegions(id, activeTrigger, states)
  ...
  for cloneId, state in pairs(states) do
    if state.show then
      local region = WeakAuras.GetRegion(id, cloneId)
      local applyChanges =
        not region.toShow or state.changed or region.state ~= state
      region.state = state
      ...
      for triggernum = -1, triggerState[id].numTriggers do
        ...
        applyChanges =
          applyChanges or region.states[triggernum] ~= triggerState or (triggerState and triggerState.changed)
        region.states[triggernum] = triggerState
        needsTimerTick =
          needsTimerTick or (triggerState and triggerState.show and triggerState.progressType == "timed")
      end
...
function WeakAuras.UpdatedTriggerState(id)
  ...
  for triggernum = 1, triggerState[id].numTriggers do
    ...
    for cloneId, state in pairs(triggerState[id][triggernum]) do
      state.trigger =
        db.displays[id].triggers[triggernum] and db.displays[id].triggers[triggernum].trigger
      state.triggernum = triggernum
      state.id = id
    ...
    changed =
      applyToTriggerStateTriggers(anyStateShown, id, triggernum) or changed
  end
  ...
  local activeTriggerState =
    WeakAuras.GetTriggerStateForTrigger(id, newActiveTrigger)
...
local function nextState(char, state)
  ...
  elseif state == 1 then -- Percent Start State
    if char == 37 then
      return 0 -- Enter Percent rest state
    -- % sign
...
symbol,
  ...
  if triggerNum and sym then
    if regionStates and regionStates[triggerNum] then
      if regionStates[triggerNum][sym] then
        return regionStates[triggerNum].show and tostring(
          regionStates[triggerNum][sym]
        ) or ""
      else
        local value =
          regionStates[triggerNum].show and ReplaceValuePlaceHolders(
            sym,
            region,
            customFunc,
            regionStates[triggerNum]
          )
        return value or ""
      end
    end
  ...
  elseif regionState and regionState[symbol] then
    ...
    local value =
      regionState and regionState.show and ReplaceValuePlaceHolders(
        symbol,
        region,
        customFunc,
        regionState
...
function WeakAuras.ReplacePlaceHolders(textStr, region, customFunc)
  ...
  if (not regionState and not regionValues) then return end
  local endPos = textStr:len()
  if (endPos < 2) then
  ...
  while currentPos <= endPos do
    ...
    elseif state == 2 then -- Percent Rest State
      if (char >= 48 and char <= 57) or (char >= 65 and char <= 90) or (char >= 97 and char <= 122) or char == 46 then -- End of variable
        -- 0-9a-zA-Z or dot character
      else
        local symbol = string.sub(textStr, start, currentPos - 1)
        result =
          result .. ValueForSymbol(
            symbol,
            region,
            customFunc,
            regionState,
...
local function ensureMouseFrame()
  ...
  local moverFrame =
    CreateFrame("FRAME", "WeakAurasMousePointerFrame", mouseFrame)
  ...
  local label =
    moverFrame:CreateFontString(nil, "BACKGROUND", "GameFontHighlightSmall")
  ...
  mouseFrame.OptionsOpened = function()
    if db.mousePointerFrame then
      ...
      mouseFrame:SetPoint(
        ...
        UIParent,
        "TOPRIGHT",
        db.mousePointerFrame.xOffset,
        db.mousePointerFrame.yOffset
      )
  ...
  mouseFrame.updateVisible = function(self)
    if not WeakAuras.IsOptionsOpen() then return end
...
local function ensurePRDFrame()
  if personalRessourceDisplayFrame then return end
  personalRessourceDisplayFrame =
    CreateFrame("FRAME", "WeakAurasAttachToPRD", UIParent)
  ...
  moverFrame:SetScript("OnMouseUp", function()
    ...
    local scale =
      UIParent:GetEffectiveScale() / personalRessourceDisplayFrame:GetEffectiveScale()
    db.personalRessourceDisplayFrame.xOffset =
      xOffset / scale - GetScreenWidth()
    db.personalRessourceDisplayFrame.yOffset =
      yOffset / scale - GetScreenHeight()
  end)
  moverFrame:Hide()
  ...
  local label =
    moverFrame:CreateFontString(nil, "BACKGROUND", "GameFontHighlight")
  ...
  personalRessourceDisplayFrame.OptionsOpened = function()
    ...
    if (KuiNameplatesCore and KuiNameplatesCore.profile) then
      ...
      if KuiNameplatesCore.profile.ignore_uiscale then
        ...
        personalRessourceDisplayFrame:SetScale(
          uiScale / UIParent:GetEffectiveScale()
        )
      else
        personalRessourceDisplayFrame:SetScale(1)
      end
      personalRessourceDisplayFrame.texture:SetTexture(
        "Interface\\AddOns\\WeakAuras\\Media\\Textures\\PRDFrameKui"
      )
    else
      ...
      prdWidth =
        baseNamePlateWidth * horizontalScale * Lerp(
          1.1,
          1.0,
          clampedZeroBasedScale
        ) - 24
      prdHeight =
        4 * namePlateVerticalScale * Lerp(
          1.2,
          1.0,
          clampedZeroBasedScale
        ) * 2 + 1
      ...
      personalRessourceDisplayFrame.texture:SetTexture(
        "Interface\\AddOns\\WeakAuras\\Media\\Textures\\PRDFrame"
      )
    end
    ...
    if (not xOffset or not yOffset) then
      ...
      xOffset =
        xPositionNextToOptions() + prdWidth / 2 / scale - GetScreenWidth()
    end
  ...
  personalRessourceDisplayFrame.OptionsClosed = function()
    ...
    personalRessourceDisplayFrame:SetScript(
      "OnEvent",
      personalRessourceDisplayFrame.eventHandler
    )
    ...
    if (event == "NAME_PLATE_UNIT_ADDED") then
      if UnitIsUnit(nameplate, "player") then
        ...
        if frame then
          ...
          personalRessourceDisplayFrame:Show()
          db.personalRessourceDisplayFrame =
            db.personalRessourceDisplayFrame or {}
        else
          personalRessourceDisplayFrame:Detach()
          personalRessourceDisplayFrame:Hide()
        end
  ...
  personalRessourceDisplayFrame.anchorFrame = function(
  self,
    id,
    anchorFrameType
  )
