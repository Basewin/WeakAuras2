...
local CreateFrame, UnitClass, UnitRace, GetSpecialization =
  CreateFrame,
  UnitClass,
  UnitRace,
  GetSpecialization
...
AceGUI:RegisterLayout("WATemplateTriggerLayoutFlyout", function(
content,
  children
)
...
local function missingBuffGreyed(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.buffedFalseAuraAlways, {
      changes("grey", regionType)
    })
  )
end
...
local function hasTargetAlpha(conditions, regionType)
  tinsert(
    conditions,
    buildCondition(nil, checks.hasTarget, { changes("alpha", regionType) })
  )
end
...
local function insufficientResourcesBlue(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.insufficientResources, {
      changes("blue", regionType)
    })
  )
end
...
local function hasChargesGrey(conditions, trigger, regionType)
  if regionType == "icon" then
    tinsert(
      conditions,
      buildCondition(trigger, checks.charges, {
        changes("cooldownSwipe", regionType)
      })
    )
  else
    tinsert(
      conditions,
      buildCondition(trigger, checks.charges, { changes("grey", regionType) })
    )
  end
end
...
local function isOnCdGrey(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.onCooldown, { changes("grey", regionType) })
  )
end
...
local function GenericGlow(conditions, trigger, regionType, check)
  if regionType == "icon" then
    tinsert(
      conditions,
      buildCondition(trigger, check, {
        changes("inverse", regionType),
  ...
  elseif regionType == "progresstexture" then
    ...
    tinsert(
      conditions,
      buildCondition(trigger, check, { changes("yellow", regionType) })
    )
  end
end
...
local function overlayGlow(conditions, trigger, regionType)
  if regionType == "icon" or regionType == "aurabar" then
    tinsert(
      conditions,
      buildCondition(trigger, checks.overlayGlow, {
        changes("glow", regionType)
      })
    )
  else
...
local function uninterruptibleRed(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.uninterruptible, {
      changes("red", regionType)
    })
  )
end
...
local function isSpellNotInRangeRed(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.spellInRange, { changes("red", regionType) })
  )
end
...
local function itemInRangeRed(conditions, trigger, regionType)
  tinsert(
    conditions,
    buildCondition(trigger, checks.itemInRange, { changes("red", regionType) })
  )
end
...
local function createBuffTrigger(triggers, position, item, buffShowOn, isBuff)
  triggers[position] = {
    trigger = {
      ...
      type = "aura2",
      useName = true,
      auranames = { tostring(item.buffId or item.spell) },
...
local function subTypesFor(item, regionType)
  ...
  local data = {}
  local dataGlow = {
    subRegions = { [subglowindex] = subglow }
  }
...
function WeakAuras.CreateTemplateView(frame)
  ...
  local function createLastPage()
    ...
    replaceButton:SetClick(function()
      ...
      WeakAuras.DeepCopy(
        WeakAuras.data_stub.load.class,
        newView.data.load.class
      )
  ...
  selectedItem -- selectedItem is either a regionType or a trigger section
  )
    ...
    if not newView.data then
      for regionType, regionData in pairs(WeakAuras.regionOptions) do
        if regionData.templates then
          local button =
            createRegionButton(regionType, regionData, selectedItem)
          newViewScroll:AddChild(button)
          if (regionType == selectedItem) then
    ...
    elseif (newView.data and not newView.choosenItem) then
      ...
      if not WeakAuras.IsClassic() then
        local specSelector =
          createDropdown("spec", WeakAuras.spec_types_specific[newView.class])
      ...
      if (WeakAuras.triggerTemplates.class[newView.class] and WeakAuras.triggerTemplates.class[newView.class][newView.spec]) then
        createTriggerButtons(
          WeakAuras.triggerTemplates.class[newView.class][newView.spec],
          selectedItem
        )
      end
      local classHeader = AceGUI:Create("Heading")
      ...
      if not WeakAuras.IsClassic() then
        ...
        if WeakAuras.triggerTemplates.items[newView.item] then
          local group =
            createTriggerFlyout(
              WeakAuras.triggerTemplates.items[newView.item].args,
              true
            )
          newViewScroll:AddChild(group)
        end
      ...
      if WeakAuras.triggerTemplates.race[newView.race] then
        local group =
          createTriggerFlyout(
            WeakAuras.triggerTemplates.race[newView.race],
            true
          )
        newViewScroll:AddChild(group)
      end
  ...
  local newViewMakeBatch =
    CreateFrame("Button", nil, newView.frame, "UIPanelButtonTemplate")
  ...
  newViewBack:SetScript("OnClick", function()
    if newView.existingAura then
      if newView.choosenSubType then
        ...
        local subTypes =
          subTypesFor(newView.choosenItem, newView.data.regionType)
  ...
  function newView.Open(self, data)
    ...
    if data then
      ...
      newView.choosenItem = nil
      newView.choosenSubType = nil -- Data is cloned from display template
    else
      self.data = nil
